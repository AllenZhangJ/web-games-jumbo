...
function safeNow(now: (() => number) | undefined) {
  try {
    const value = now?.();
...
function platformError(id: string, message: string, cause?: unknown) {
  const error = new Error(`[${id}] ${message}`);
  if (cause) error.cause = cause;
  return error;
}

export function normalizeCanvasSize(width: unknown, height: unknown, id = 'unknown') {
...
function installEventTargetFallback(canvas: any, id: string) {
...
export function prepareCanvas(canvas: any, id = 'unknown'): any {
...
export function sizeCanvas(canvas: any, width: unknown, height: unknown, id = 'unknown'): any {
...
export function createFrameScheduler({
  ...
  request?: ((callback: (time: number) => void) => unknown) | undefined;
  cancel?: ((id: unknown) => void) | undefined;
...
function reportsWebGL2(context: any) {
...
function validateWebGL2Context(context: any, id: string) {
...
export function getRequiredWebGL2Context(canvas: any, attributes: any = {}, id = 'unknown'): any {
  prepareCanvas(canvas, id);
...
export function createPlatformContract(overrides: any = {}): any {
  ...
  return {
    ...
    getWebGLContext: (canvas: any, attributes: any) => getRequiredWebGL2Context(canvas, attributes, id),
    createImage: () => null,
