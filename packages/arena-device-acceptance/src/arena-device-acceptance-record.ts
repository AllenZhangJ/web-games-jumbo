...
export type ArenaDeviceAcceptanceCheckResult =
  typeof ARENA_DEVICE_ACCEPTANCE_CHECK_RESULT[keyof typeof ARENA_DEVICE_ACCEPTANCE_CHECK_RESULT];

export interface ArenaDeviceAcceptanceClient {
  readonly name: string;
  readonly version: string;
  readonly baseLibraryVersion: string | null;
}
export interface ArenaDeviceAcceptanceDevice {
  readonly manufacturer: string;
  readonly model: string;
  readonly osName: string;
  readonly osVersion: string;
}
export interface ArenaDeviceAcceptanceArtifact {
  readonly id: string;
  readonly kind: ArenaDeviceAcceptanceArtifactKind;
  readonly path: string;
  readonly sha256: string;
  readonly byteLength: number;
}
export interface ArenaDeviceAcceptanceCheckRecord {
  readonly id: string;
  readonly result: ArenaDeviceAcceptanceCheckResult;
  readonly notes: string;
  readonly artifactIds: readonly string[];
}
export interface ArenaDeviceAcceptanceRecord {
  readonly schemaVersion: typeof ARENA_DEVICE_ACCEPTANCE_RECORD_SCHEMA_VERSION;
  ...
  readonly client: ArenaDeviceAcceptanceClient;
  readonly device: ArenaDeviceAcceptanceDevice;
  readonly orientation: 'portrait';
  readonly inputMode: 'touch';
  readonly checks: readonly ArenaDeviceAcceptanceCheckRecord[];
  readonly artifacts: readonly ArenaDeviceAcceptanceArtifact[];
}

const RECORD_KEYS = new Set([
  'schemaVersion', 'recordId', 'definitionId', 'definitionHash', 'commit', 'buildId',
  'targetId', 'runId', 'performedAt', 'operatorId', 'client', 'device', 'orientation',
  'inputMode', 'checks', 'artifacts',
]);
...
function exactValue<T extends string>(value: unknown, expected: T, name: string): T {
...
function nullableBoundedString(value: unknown, maximumLength: number, name: string): string | null {
  return value === null ? null : boundedString(value, maximumLength, name);
}

function cloneClient(value: unknown, target: ArenaDeviceAcceptanceTarget): ArenaDeviceAcceptanceClient {
  const name = 'ArenaDeviceAcceptanceRecord.client';
  assertKnownKeys(value, CLIENT_KEYS, name);
  const baseLibraryVersion = nullableBoundedString(value.baseLibraryVersion, 128, `${name}.baseLibraryVersion`);
  if (target.platform === ARENA_DEVICE_ACCEPTANCE_PLATFORM.WEB && baseLibraryVersion !== null) {
...
function cloneDevice(value: unknown, target: ArenaDeviceAcceptanceTarget): ArenaDeviceAcceptanceDevice {
  ...
  if (target.requiredOsNames && !target.requiredOsNames.includes(device.osName)) {
    throw new RangeError(`target ${target.id} 只接受系统：${target.requiredOsNames.join('、')}。`);
  }
  return device;
}
...
function cloneArtifacts(
  values: unknown,
  target: ArenaDeviceAcceptanceTarget,
): readonly ArenaDeviceAcceptanceArtifact[] {
  if (!Array.isArray(values) || values.length === 0) {
    throw new RangeError('ArenaDeviceAcceptanceRecord.artifacts 不能为空。');
  }
  if (values.length > MAXIMUM_ARTIFACTS) {
    throw new RangeError(`ArenaDeviceAcceptanceRecord.artifacts 不能超过 ${MAXIMUM_ARTIFACTS} 项。`);
  }
  const ids = new Set<string>();
  const paths = new Set<string>();
  const artifacts = values.map((value: unknown, index): ArenaDeviceAcceptanceArtifact => {
    ...
    if (paths.has(artifactPath)) throw new RangeError(`同一运行不能重复引用 artifact 路径 ${artifactPath}。`);
  ...
  for (const kind of target.requiredArtifactKinds) {
    if (!kinds.has(kind)) throw new RangeError(`target ${target.id} 缺少 ${kind} 证据。`);
  }
...
function cloneChecks(
  values: unknown,
  target: ArenaDeviceAcceptanceTarget,
  artifacts: readonly ArenaDeviceAcceptanceArtifact[],
): readonly ArenaDeviceAcceptanceCheckRecord[] {
  if (!Array.isArray(values)) throw new TypeError('ArenaDeviceAcceptanceRecord.checks 必须是数组。');
  const required = new Set(target.requiredCheckIds);
  const artifactIds = new Set(artifacts.map(({ id }) => id));
  const ids = new Set<string>();
  const checks = values.map((value: unknown, index): ArenaDeviceAcceptanceCheckRecord => {
    ...
    ids.add(id);
    const referencedArtifacts = cloneFrozenStringSet(value.artifactIds as readonly unknown[], `${name}.artifactIds`);
    if (referencedArtifacts.length === 0) throw new RangeError(`${name}.artifactIds 不能为空。`);
    for (const artifactId of referencedArtifacts) {
      if (!artifactIds.has(artifactId)) throw new RangeError(`${name} 引用未知 artifact ${artifactId}。`);
    }
    return Object.freeze({
      id,
...
export function createArenaDeviceAcceptanceRecord(
  definitionValue: unknown,
  value: unknown,
): ArenaDeviceAcceptanceRecord {
  const definition: ArenaDeviceAcceptanceDefinition = createArenaDeviceAcceptanceDefinition(definitionValue);
  ...
  if (source.schemaVersion !== ARENA_DEVICE_ACCEPTANCE_RECORD_SCHEMA_VERSION) {
    throw new RangeError(`不支持 ArenaDeviceAcceptanceRecord schema ${String(source.schemaVersion)}。`);
  }
  if (source.definitionId !== definition.id) throw new RangeError('ArenaDeviceAcceptanceRecord.definitionId 与当前定义不一致。');
  const definitionHash = definition.getContentHash();
  if (source.definitionHash !== definitionHash) throw new RangeError('ArenaDeviceAcceptanceRecord.definitionHash 与当前定义不一致。');
  ...
  if (unreferencedArtifact) throw new RangeError(`artifact ${unreferencedArtifact.id} 未被任何 check 引用。`);
  return Object.freeze({
    ...
    definitionHash,
    commit: assertEvidenceGitCommit(source.commit, 'ArenaDeviceAcceptanceRecord.commit'),
    ...
    performedAt: assertEvidenceUtcInstant(source.performedAt, 'ArenaDeviceAcceptanceRecord.performedAt'),
    ...
    orientation: exactValue(source.orientation, 'portrait', 'ArenaDeviceAcceptanceRecord.orientation'),
    inputMode: exactValue(source.inputMode, 'touch', 'ArenaDeviceAcceptanceRecord.inputMode'),
    checks,
    artifacts,
  });
}

export function isArenaDeviceAcceptanceRecordPassing(record: ArenaDeviceAcceptanceRecord): boolean {
  return record.checks.every(({ result }) => result === ARENA_DEVICE_ACCEPTANCE_CHECK_RESULT.PASSED);
}
