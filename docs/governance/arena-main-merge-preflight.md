...
当前结论是 **不可直接合并**。这不表示 Arena 治理路线失败，而是说明最新 `main` 与审计候选已经形成两个互斥产品方向：`main` 的 12 个新增提交继续治理已退役的数值跳台，截至代码候选当前治理分支的 442 个新增提交把 Arena 建成唯一生产产品并承接适用的主干治理意图。普通自动合并会在入口、构建、配置、测试和治理真值上重新引入旧实现。
...
| 治理代码候选 | `2f28df1bc7082d5ca1759f4c14547e6277ce4d68` |
| main 独有非 merge 提交 | 12 |
| 截至代码候选的治理分支独有提交 | 442 |
| 文本虚拟合并冲突 | 23 个文件 |

候选 `2f28df1` 在 `9e1460b` 的 clean checkout 修复和显式审计边界上，继续吸收 `main` 的包级 ESLint、公开命令前置构建和启动失败测试意图，并删除零引用旧 `src/config.ts`。其隔离 clean build ID 为 `arena-2f28df1bc708-product`，Web/微信/抖音 Manifest hash 分别为 `ccc95c17`、`bd96d30c`、`403604d6`。
...
因此，`main` 的治理思想已经逐项审计并承接适用部分，但其具体代码、旧产品文档、旧入口、旧配置、旧基线和旧资产审计不能进入 Arena 生产树。23 个文件的逐项裁决、已吸收能力和未来集成顺序见 [冲突处置矩阵](arena-main-conflict-disposition.md)。
...
其中 `docs/architecture.md`、`docs/gameplay-rules.md` 和 `src/config.ts` 的最终裁决是保持删除；入口、构建、配置和根 manifest 以 Arena 为产品真值；ESLint、公开命令前置构建与启动失败测试已把 `main` 的适用治理意图重新表达为 Arena 实现。它们仍不能按文本相似度自动裁决。

## 候选验证

- 在新的无历史 `dist`/`.tsbuildinfo` 隔离克隆中，项目 `audit=false`，`npm ci --ignore-scripts --no-audit` 后直接执行公开 `npm test` 和 `check:governance` 均通过：52 个 workspace 包按 11 个依赖波次构建，61 个 Vitest 文件、385 项测试通过；语句/行 `59.99%`、分支 `64.18%`、函数 `64.29%`。
...
- 三端 clean build：build ID `arena-2f28df1bc708-product`，默认入口均为 Product，`sourceDirty=false`，产物边界和预算通过；Web/微信/抖音 delivery 为 `3807531 / 3835130 / 3835105 B`。
...
- 截至审计时，最新已推送候选 `3fd4be5` 的 GitHub Actions 运行 [30002587702](https://github.com/AllenZhangJ/web-games-jumbo/actions/runs/30002587702) 失败。本地已在隔离克隆中复现并修复 clean-install 根因，但当前代码候选 `2f28df1` 尚未推送，因此不得将本地通过写成远端 CI 通过。
...
冲突处置方法本身已经在 [逐文件矩阵](arena-main-conflict-disposition.md) 中审计完成；第 4 项剩余的是未来对实际集成批次的独立授权与执行后证据，不再是“尚未分析冲突”。
