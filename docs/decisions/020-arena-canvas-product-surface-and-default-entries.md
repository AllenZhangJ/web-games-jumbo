...
`createProductUiSceneModel(viewModel)` 只把公开 ViewModel 投影为标题、文案、角色卡、对手名、奖励和动作数据。它位于 `presentation/product`，不包含 Three、DOM、Canvas、平台 API 或资产 URL。
...
- `npm run build` 生成 Product 发行包；`npm run build:greybox` 独立生成可验证、可紧急回退的灰盒发行包。Manifest 的 `defaultEntry` 固定本包入口身份。
- `ARENA_MINI_ENTRY_MODE=greybox npm run build`（或 `npm run build:greybox`）会让最终 `game.js` 精确复制灰盒 bundle；默认 `npm run build` 复制产品 bundle。
