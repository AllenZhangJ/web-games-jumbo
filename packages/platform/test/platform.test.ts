...
import { createMiniGamePlatform } from '../src/mini-game.js';
import { createFrameScheduler, createPlatformContract } from '../src/platform-contract.js';
import { createWebPlatform } from '../src/web.js';
...
test('Web storage and share failures return safe values without rejection', async () => {
  ...
  assert.equal(platform.storageRemove('save'), true);
  assert.deepEqual(removed, ['save']);
  assert.equal(await platform.share({ title: 'test' }), false);
});

test('mini-game storage removal uses the host removal API instead of writing undefined', () => {
  const fixture = miniGameApi({ id: 'wechat' });
  const removed: string[] = [];
  fixture.api.removeStorageSync = (key) => { removed.push(key); };
  fixture.api.setStorageSync = () => { throw new Error('must not write'); };
  const platform = createMiniGamePlatform(fixture.api, fixture.id);
  assert.equal(platform.storageRemove('save'), true);
  assert.deepEqual(removed, ['save']);
});
