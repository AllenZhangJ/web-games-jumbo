...
| --- | --- | ---: | ---: | --- | --- |
| `equipment.scripted-pressure` | 三件装备拾取/动作/命中 | 2584805376 | 899 | `17b60bcb` | `c9cd7e73` |
| `lifecycle.quick-match-pause-resume` | QuickMatch/Session start 前与局中暂停 | 1375600641 | 479 | `543a7a80` | `33a33688` |
| `map.first-wind-cycle` | 风场预警/开始/结束 | 1517158400 | 1199 | `2e092bc6` | `389b7142` |
| `movement.semantic-actions`（场景 v2） | 地面跳、二段跳、蹲跳、空中攻击/落地 | 1834352640 | 899 | `8673e0bf` | `e560dd88` |

Manifest hash 为 `a53b401d`。movement 场景 v2 以固定 12 方向有理向量和量化点积替代运行时三角函数，消除 Linux/macOS 数学库末位差异；设计与替代方案见 [ADR-041](../decisions/041-cross-platform-movement-replay-directions.md)。完整 JSON 有意保留输入、checkpoint、事件与结果的人工可审查性；只有仓库体积出现可测量问题时才评估压缩归档格式。
...
项目已有失败软化、固定容量 voice 池和幂等销毁的命中音频运行时，并覆盖 Renderer 去重与销毁；目标宿主前后台音频中断/恢复仍没有正式设备 Record，不将其标记为通过。

## S9.6b3 原子回归证据

`arena.stage9.regression-evidence.v1` 将日常 shell 聚合升级为可交接的机器合同，固定两套 Mapper 各 40 局 fuzz/2 条抽样回放、六个 lifecycle 测试文件、两条 100 局 Session soak、8 MiB 回收后堆增长预算和 200 局 Product stress。

生成器直接 spawn Node，不使用 shell；每项必须输出完整 TAP 或唯一 JSON，非零退出、stderr、超时、输出超限、部分执行、资源残留或超预算都不会生成 Report。Report 只能在 clean checkout 上向仓库外的新 `.json` 独占原子发布，并由 `arena:stage9:readiness` 再次复算 Definition/result hash 和 candidate commit。黄金回放已有独立 Gate，因此不在该 Report 内重复执行。
...
# 只允许 clean checkout，且 output 必须位于仓库外并且不存在
npm run arena:regression:evidence -- --output /tmp/arena-regression-evidence.json
...
- 全量测试：61 个 Vitest 文件/387 项与 88 个 Node 文件/706 项通过。
- 黄金语料：4/4 严格重放与再生成，Manifest hash `a53b401d`。
- input fuzz：三套 Mapper 各 40 局，120/120 唯一最终 hash，6/6 抽样严格回放。
- 生命周期回归：104/104 通过。
- 旧表现 Session soak：100/100，最终 frame/listener/input 所有权均为 0/未绑定，GC 后堆增长 2,702,632 B，低于 8 MiB 预算。
- Product Presentation Session soak：100/100、100 个唯一 Authority hash，最终资源所有权清零，GC 后堆增长 6,885,824 B，低于 8 MiB 预算。
- Web、微信、抖音三端 clean build 与 Manifest 验证通过，build ID `arena-a71ecc1c0493-product`、`sourceDirty=false`；目标真机 Record 仍按独立门禁管理。
