...
执行中。本文定义 Stage 6 的完成证据，不表示后续批次已经通过。S6.1～S6.6.2 已分批完成本机门禁；Web 真实浏览器和 100 局 Session soak 已通过。S6.6.2 已有单写入入组账本、真实消费输入观察和 active-tick 自动指标，但平台存储、采集入口与真实样本尚未完成；微信/抖音开发者工具和目标真机 E3 也仍未通过。实现边界见 [Stage 6 输入、移动与灰盒执行计划](../architecture/arena-stage6-input-movement-plan.md)，决策背景见 [ADR-009](../decisions/009-arena-semantic-input-and-movement-authority.md)。
...
S6.6.2 当前本机运行时证据见 [S6.6.2 盲测运行时门禁记录](../research/arena-stage6-input-pilot-runtime.md)：assignment 先持久化再提交、失败原子回滚、真实已消费 InputFrame 观察、暂停不计时、authority 事件成功判定、一次 assignment 一局和观察失败关闭均有测试；1,000 局 Core 压测仍通过原有 CPU/堆预算。平台存储 CAS、跨刷新恢复、独立入口与 E4 仍是阻断项。
