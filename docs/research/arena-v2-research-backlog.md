...
当前新增证据：四类代表 KZ 地图样本已经形成结构化研究卡，分别覆盖长跳练习、长路线检查点、垂直攀爬恢复和混合移动标签；六段灰盒又通过同一套 Rule/Physics 武器攻击探针完成 18 个段落×武器组合，证明同一把武器在宽平台、迷宫、窄路和走钢丝上的后果不同；详见[CS1.6 KZ 跳跃地图研究 V1](arena-v2-cs16-kz-map-study-v1.md)、`arena-v2-kz-map-research-catalog.ts` 和 `arena-v2-kz-route-combat-prototype.ts`。下一步仍需加入移动/跳跃回应和真人视野验证，不引入第三方地图资产。
