...
function material(color: number, { metalness = 0.08, roughness = 0.72 } = {}): THREE.MeshStandardMaterial {
...
function createHammer(): THREE.Group {
  ...
  const handle = mesh(new THREE.CylinderGeometry(0.055, 0.078, 1.08, 12), grip);
  handle.position.y = -0.5;
  const head = mesh(new RoundedBoxGeometry(0.62, 0.38, 0.42, 4, 0.055), darkSteel);
  head.position.y = -1.02;
  const leftFace = mesh(new THREE.CylinderGeometry(0.265, 0.23, 0.23, 12), steel);
...
function createShield(): THREE.Group {
  ...
  const shield = mesh(new THREE.CylinderGeometry(0.43, 0.43, 0.105, 16), material(
    ARENA_GREYBOX_COLOR.teal, { metalness: 0.15, roughness: 0.5 },
  ));
  shield.rotation.x = Math.PI / 2;
  const boss = mesh(new THREE.SphereGeometry(0.1, 8, 6), material(
    ARENA_GREYBOX_COLOR.equipment, { metalness: 0.25, roughness: 0.5 },
  ));
  boss.position.z = 0.09;
  const warningBar = mesh(new THREE.BoxGeometry(0.56, 0.11, 0.045), material(
...
function createChain(): THREE.Group {
  const root = new THREE.Group();
  root.name = 'ArenaWeapon:Chain';
  const chainMaterial = material(ARENA_GREYBOX_COLOR.equipment, { metalness: 0.35, roughness: 0.45 });
  ...
  const hook = mesh(new THREE.TorusGeometry(0.18, 0.055, 7, 12, Math.PI * 1.45), material(
