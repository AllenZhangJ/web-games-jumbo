...
describe('Human Match Study strict foundation', () => {
  ...
  it('rejects capture package accessors without evaluating untrusted code', () => {
    let reads = 0;
    const capturePackage = Object.defineProperty({}, 'matches', {
      enumerable: true,
      get() {
        reads += 1;
        return [];
    ...
    expect(() => createHumanMatchStudyCapturePackage(
      createArenaStage9HumanFairnessV1Definition(),
      capturePackage,
    )).toThrow(/访问器|数据字段/);
    expect(reads).toBe(0);
  ...
  it('rejects controller option accessors without evaluating them', () => {
    let reads = 0;
    const options = Object.defineProperty({}, 'definition', {
      enumerable: true,
      get() {
