...
- 状态：研究工具链原型通过；白金双枪、血影钩刃与幻虎巨拳已接入研究 Definition 数值投影，其余三件仍待候选 Definition
- 日期：2026-07-28
...
当前生产武器概览已经能够从权威 `Definition` 投影 9 个主数值轴、2 个行为轴和地面/空中上下文。六件逐动作深研案例中，白金双枪已经映射到 `research-line-pressure`，血影钩刃和幻虎巨拳已经有独立研究 Definition 与 Replay；幻虎巨拳另有边缘结果，血影钩刃另有独立障碍探针；其余三件仍只有官方事实、设计推导、反制、失败成本和数值审计，不能填入假数值。
...
血影钩刃的方向分支已经由 `rear-cone` 与目标主动转身 Replay 验证；障碍阻挡仍由独立二维探针验证，不会被伪装成现有 9 个主比较轴。障碍依赖仍需接入真实地图表面、空中高度和复活重新进入路线后，才有资格讨论是否新增玩家可读字段。
...
- `arena-v2-weapon-case-study-overview-prototype.test.ts`、血影钩刃 Definition/Replay 测试：相关 9 项测试通过；
...
1. 为真·哈迪斯钩镰、魔血镰刃和猛犸石斧分别建立最小候选 Definition；
2. 对每件候选的地面/空中动作测出主 9 轴和行为 2 轴，不能凭官方原作数值换算；
3. 将每件投影接入统一案例概览和可读性矩阵，再进行真人首见时间、命中归因和反制可读性测试；
