...
旧入口仍可执行：Web 提供 `/greybox.html`；小游戏通过 `npm run build:greybox` 生成独立灰盒 `game.js` 发行包。默认构建只交付 Product `game.js`，不重复携带未启用入口。
...
- `npm run build:greybox` 后，两端 Manifest 的 `defaultEntry` 为 `greybox`；默认构建为 `product`。两种构建都只包含一个权威 `game.js`。
