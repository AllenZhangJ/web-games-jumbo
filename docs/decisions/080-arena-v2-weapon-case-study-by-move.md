...
1. 每件深入研究的武器必须按动作保存：官方事实、设计推导、玩家决策、反制、失败成本和数值审计状态；魔血镰刃、真·哈迪斯钩镰、白金双枪、血影钩刃、幻虎巨拳和猛犸石斧共同使用这一合同。
...
6. 逐件案例可以生成研究概览适配层；未连接候选 `Definition` 的案例必须标记为 `not-yet-available`，已连接的案例必须标记为 `research-projection`，同时携带来源 Definition、地面/空中投影和比较集合。两种状态都不能被当作生产玩家数值。
...
- `packages/arena-v1-experiment/src/arena-v2-weapon-phantom-tiger-fist-case-study.ts`
- `packages/arena-v1-experiment/test/arena-v2-weapon-phantom-tiger-fist-case-study.test.ts`
