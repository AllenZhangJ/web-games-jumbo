...
2026-07-22 的治理实现把构建预算 Policy/Report 及性能 Policy/Record/Metric/Report 迁入 strict `@number-strategy-jump/arena-performance-evidence`。该包只依赖不可变数据、证据标量与设备验收合同，只重算调用方注入的 Manifest/Trace；架构门禁止它读取宿主计时、DOM、Three.js、平台 API、随机源或网络。运行时采集继续由 `arena-presentation-runtime` 的注入式观察器负责，证据包不持有其生命周期。Stage 9 六 target Policy、Device Definition、Definition Catalog 和组合 Report 进一步迁入 strict `@number-strategy-jump/arena-stage9-evidence-content`，该上层包只组合已验证合同与质量 Definition，不持有采集或宿主生命周期。代码合同已收口仍不等于 S9.4 关闭：六个真实手机 target 的 clean-build Record 未齐前，状态保持“待采集”。
