...
S6.1～S6.6.4 已完成对应本机门禁；Web 真实浏览器桌面/竖屏、结果/重赛闭环和 100 局 Session soak 已完成。S6.6.1 已建立盲测合同与确定性区组分配；S6.6.2 已建立只读运行时采集；S6.6.3a 已建立版本化双槽工作区；S6.6.3b/c 已建立 Trial Controller、终态提交、可恢复复核草稿、去标识导出与独立 Web 采集工作台；S6.6.4 已建立与构建绑定的 E3 证据合同和附件验证器。真实新手 E4 样本、微信/抖音开发者工具与目标真机 E3 仍未执行，因此 Mapper 胜者和手势阈值仍未冻结。
...
- 微信、抖音目前只有构建和宿主合同 E2；五个 E3 target 已有统一 Definition/Record/Bundle/Report 合同和校验 CLI，但尚无开发者工具、目标真机触控、前后台、WebGL context loss 与安全区 Record。
...
- InputFrame V4、Replay V5（S8.4 增加冻结内容选择）、state hash 与旧版本拒绝测试。
...
- 状态：S6.6.1 已建立 Definition、固定 seed 区组分配与聚合报告；S6.6.2 已建立真实消费输入观察和自动指标；S6.6.3a 已建立三端 Storage、Workspace、双槽 Repository 与协作 lease，详见 [持久化基础门禁](../research/arena-stage6-input-pilot-persistence.md) 和 [ADR-013](../decisions/013-arena-pilot-local-evidence-workspace.md)。S6.6.3b/c 已落地 Trial Controller、单次终态提交、可恢复复核草稿、导出和独立 Web 工作台；S9.6b5b 进一步补齐 build 隔离 Evidence Bundle、共享 verifier 和 E3 release 绑定，详见 [正式证据手册](../acceptance/stage6-input-pilot/README.md)。待完成项收窄为目标设备 E3、真人新手 E4 和冻结决策。
...
- 状态：E3 机器可验证合同已实现，见 [ADR-014](../decisions/014-arena-versioned-device-acceptance-evidence.md) 与 [验收操作手册](../acceptance/stage6/README.md)；本项不包含真实设备 Record 或 E4 样本。
- Device Acceptance Definition/Record/Bundle/Report 位于 Presentation/Acceptance，只保存纯数据，不进入 Core、Replay、Session 或 Pilot Workspace。
- CLI 输出固定 Definition/hash，并验证当前 commit/build、目标覆盖、附件路径、字节数与 SHA-256；不完整、失败和非法证据使用不同退出码。
- E3 全绿后才开始计入 E4 新手样本；E3 与 E4 独立归档，避免设备缺陷污染发现性指标或参与者数据进入设备 Manifest。

门禁：合同与 CLI 的敌意输入、混合构建、失败抵消、附件篡改和符号链接逃逸测试通过；五个真实 target 仍必须由人工执行并审看原始证据。
