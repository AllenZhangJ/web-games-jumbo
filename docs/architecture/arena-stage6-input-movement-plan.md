...
S6.1～S6.6.3a 已完成对应本机门禁；Web 真实浏览器桌面/竖屏、结果/重赛闭环和 100 局 Session soak 已完成。S6.6.1 已建立版本化盲测合同、确定性区组分配和聚合报告；S6.6.2 已建立单写入账本、MatchSession 观察装饰器和 active-tick 自动指标；S6.6.3a 已建立三端 Storage 结果合同、版本化 Workspace、双槽 Repository、协作 lease 与跨刷新 checkpoint。Trial Controller、终态提交、独立采集入口与真实新手样本尚未开始。微信、抖音开发者工具与目标真机 E3 仍未执行，因此 Mapper 胜者和手势阈值仍未冻结。
...
- 状态：S6.6.1 已建立 Definition、固定 seed 区组分配、原始记录校验和只给出候选的聚合报告，详见 [S6.6 输入盲测合同记录](../research/arena-stage6-input-pilot-contract.md)。S6.6.2 已建立并通过单写入入组账本、真实消费输入观察和自动指标的本机门禁，详见 [S6.6.2 盲测运行时门禁记录](../research/arena-stage6-input-pilot-runtime.md)。S6.6.3a 已建立三端 Storage 结果合同、版本化 Workspace、双槽读回、协作 lease、损坏/未来版本策略与 A/B 同 block match seed，详见 [S6.6.3a 持久化基础门禁记录](../research/arena-stage6-input-pilot-persistence.md) 和 [ADR-013](../decisions/013-arena-pilot-local-evidence-workspace.md)。Trial Controller、终态提交、独立采集入口和真实样本仍待实现。
