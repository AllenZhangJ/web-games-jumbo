...
S8.5.1 已建立无宿主产品表现基础：产品状态由版本化 Screen Definition、本地化 Message Catalog 和内容表现 Registry 投影为只读 ViewModel；页面输入通过串行 Intent Dispatcher 进入 ProductController；真实比赛通过非拥有 Match 桥复用既有 Arena frame projector，同时保持 ProductController 是唯一 Match 所有者。

本批不是 S8.5 完成声明。Web/微信/抖音正式页面、产品 FrameLoop、自动奖励提交、真实 App 生命周期、无障碍宿主绑定和真机留证尚未接入。
...
- `results -> commitReward -> reward` 的自动编排、保存失败重试和 UI 更新原子性。
