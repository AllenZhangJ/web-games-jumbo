...
import type { ArenaDeviceAcceptanceRecord } from './arena-device-acceptance-record.js';
...
export type ArenaDeviceAcceptanceReportStatus =
  typeof ARENA_DEVICE_ACCEPTANCE_REPORT_STATUS[keyof typeof ARENA_DEVICE_ACCEPTANCE_REPORT_STATUS];

export interface ArenaDeviceAcceptanceBundle {
  readonly schemaVersion: typeof ARENA_DEVICE_ACCEPTANCE_BUNDLE_SCHEMA_VERSION;
  readonly definitionId: string;
  readonly definitionHash: string;
  readonly commit: string;
  readonly buildId: string;
  readonly createdAt: string;
  readonly records: readonly ArenaDeviceAcceptanceRecord[];
}
export interface ArenaDeviceAcceptanceTargetReport {
  ...
  readonly status: ArenaDeviceAcceptanceReportStatus;
}

const BUNDLE_KEYS = new Set([
  'schemaVersion', 'definitionId', 'definitionHash', 'commit', 'buildId', 'createdAt', 'records',
]);
...
export function createArenaDeviceAcceptanceBundle(
  ...
  if (source.schemaVersion !== ARENA_DEVICE_ACCEPTANCE_BUNDLE_SCHEMA_VERSION) {
    throw new RangeError(`不支持 ArenaDeviceAcceptanceBundle schema ${String(source.schemaVersion)}。`);
  }
  if (source.definitionId !== definition.id) throw new RangeError('ArenaDeviceAcceptanceBundle.definitionId 与当前定义不一致。');
  const definitionHash = definition.getContentHash();
  if (source.definitionHash !== definitionHash) throw new RangeError('ArenaDeviceAcceptanceBundle.definitionHash 与当前定义不一致。');
  const commit = assertEvidenceGitCommit(source.commit, 'ArenaDeviceAcceptanceBundle.commit');
  const buildId = boundedString(source.buildId, 128, 'ArenaDeviceAcceptanceBundle.buildId');
  if (!Array.isArray(source.records)) throw new TypeError('ArenaDeviceAcceptanceBundle.records 必须是数组。');
  ...
  const artifactPaths = new Map<string, { readonly runId: string; readonly kind: string }>();
  const buildManifestsByPlatform = new Map<string, { readonly sha256: string; readonly byteLength: number }>();
  const records = source.records.map((value: unknown, index): ArenaDeviceAcceptanceRecord => {
    const record = createArenaDeviceAcceptanceRecord(definition, value);
    if (record.commit !== commit) throw new RangeError(`records[${index}].commit 与 bundle commit 不一致。`);
    if (record.buildId !== buildId) throw new RangeError(`records[${index}].buildId 与 bundle buildId 不一致。`);
    if (recordIds.has(record.recordId)) throw new RangeError(`重复的设备证据 recordId ${record.recordId}。`);
    if (runIds.has(record.runId)) throw new RangeError(`重复的设备证据 runId ${record.runId}。`);
    recordIds.add(record.recordId);
    runIds.add(record.runId);
    const target = definition.getTarget(record.targetId);
    if (!target) throw new RangeError(`未知设备验收 target ${record.targetId}。`);
    for (const artifact of record.artifacts) {
      const previous = artifactPaths.get(artifact.path);
      const sharedBuildManifest = previous?.kind === ARENA_DEVICE_ACCEPTANCE_ARTIFACT_KIND.BUILD_MANIFEST
        && artifact.kind === ARENA_DEVICE_ACCEPTANCE_ARTIFACT_KIND.BUILD_MANIFEST;
      if (previous !== undefined && !sharedBuildManifest) {
        throw new RangeError(`artifact 路径 ${artifact.path} 被 ${previous.runId} 与 ${record.runId} 重复使用。`);
      }
      ...
      if (artifact.kind === ARENA_DEVICE_ACCEPTANCE_ARTIFACT_KIND.BUILD_MANIFEST) {
        ...
        if (previousManifest !== undefined
          ...
          throw new RangeError(`平台 ${target.platform} 的 Record 必须引用同一构建 Manifest。`);
        }
  ...
  const createdAt = assertEvidenceUtcInstant(source.createdAt, 'ArenaDeviceAcceptanceBundle.createdAt');
...
export function createArenaDeviceAcceptanceReport(definitionValue: unknown, bundleValue: unknown) {
  ...
  const targets: readonly ArenaDeviceAcceptanceTargetReport[] = definition.targets.map((target) => {
    ...
    let status: ArenaDeviceAcceptanceReportStatus = ARENA_DEVICE_ACCEPTANCE_REPORT_STATUS.INCOMPLETE;
    if (failingRuns > 0) status = ARENA_DEVICE_ACCEPTANCE_REPORT_STATUS.FAILED;
    else if (passingRuns >= target.minimumPassingRuns) status = ARENA_DEVICE_ACCEPTANCE_REPORT_STATUS.READY;
    return Object.freeze({
      targetId: target.id,
  ...
  const failingTargetIds = targets
    ...
    .map(({ targetId }) => targetId);
  const status = targets.every(({ status: targetStatus }) => targetStatus === ARENA_DEVICE_ACCEPTANCE_REPORT_STATUS.READY)
  ...
  return cloneFrozenData({
    ...
    failingRunCount: bundle.records.filter((record) => !isArenaDeviceAcceptanceRecordPassing(record)).length,
    missingTargetIds,
    failingTargetIds,
    targets,
