...
| `arena-v2-weapon-case-study-overview-prototype.ts` | 将六件逐动作深研案例投影为核心动词、上下文、公共轴状态、地图信号和反制读出；当前五件已连接研究 Definition 地面/空中数值投影，未连接案例禁止假数值 | 深研概览适配层，仅研究工具链 |
...
| [深研武器概览适配层原型结果 V1](../research/arena-v2-weapon-case-study-overview-prototype-results-v1.md) | 将六件逐动作深研案例的研究结论与公共数值轴审计接到同一份读出，区分五件已有研究投影与一件未连接案例 | 研究读出通过，未进入生产 UI |
...
| [武器数值可读性任务原型结果 V1](../research/arena-v2-weapon-readability-task-prototype-results-v1.md) | 在题目前展示同源地面/空中矩阵，再生成五项比较/方向/地图/上下文任务，隔离参与者题目与研究员答案并绑定矩阵哈希 | 研究页已接入并完成桌面验证，真人样本尚无 |
