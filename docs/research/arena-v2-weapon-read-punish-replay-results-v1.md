...
- 状态：研究候选已通过统一动作状态与 MatchReplay 验证，仍未进入生产内容
- 日期：2026-07-28
...
本原型把读招反制从独立整数 tick 探针接入真实 `MatchCore → ArenaRuleEngine → ActionExecutionSystem → MatchReplay` 链路。它只验证承诺状态和可回放行为，不新增正式武器 ID、存档字段、页面或生产 `EquipmentDefinition`。

## 固定 Definition 语义

读招反制地面动作使用 research-only `ActionDefinition`：
...
|---|---:|---|---:|---|---|
| 提前释放 | tick 1 | tick 9 取消 | 无 | `5167a22b` | 提前松开承担空放结果，但不会进入 active |
| 成功提交 | tick 1 | tick 13 提交 | tick 25 | `0cf78693` | 达到 12 tick 后释放，正式进入有效窗口并命中 |
| 到期持续按住 | tick 1 | tick 19 取消 | 无 | `5167a22b` | 等过承诺窗口会取消，不自动替玩家释放 |
...
仍阻塞：正式 Definition、正式反馈表现和生产迁移。研究候选继续留在开发/测试工具链，不能被玩家获得。
