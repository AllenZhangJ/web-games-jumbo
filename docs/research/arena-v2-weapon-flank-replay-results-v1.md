...
- 状态：候选 Definition 已通过目标主动转身、多次转身、侧向进入与统一动作 Replay 验证，仍未进入默认生产内容
- 日期：2026-07-28
- 候选：`launch-06-flank`
...
本原型使用真实 `MatchCore`、普通移动输入、`ArenaRuleEngine` 和 `MatchReplay`，验证绕后不是固定“目标背向就必定命中”的脚本：目标保持背向时命中，目标在 active 前转身后避开，攻击者也能从侧向进入目标背后后命中。
...
| 侧向进入后转入背后 | 攻击者从 `(1.2, 1.5)` 沿 `moveZ=-1` 接近，tick 24 出手 | `-1` | tick 34 | `ef8764cd` | 从 rear-cone 外的 `-0.625` 进入 active 前的 `-0.800`，命中成立 |
...
地图后果此前已经通过六段 KZ 灰盒探针；双人边缘 Replay、研究反馈来源和侧向进入已补齐。多目标视线遮挡已通过独立研究探针验证观察负担，但仍不是三人/网络多人命中规则。默认生产注册、正式生产反馈资产和真人可读性仍阻塞，因此不会进入默认生产 `EquipmentRegistry`。

## 后续

1. 用[多目标视线遮挡研究原型结果 V1](arena-v2-weapon-occlusion-research-prototype-results-v1.md)继续验证近/远目标反馈，但不把研究目标伪装成生产参与者；
