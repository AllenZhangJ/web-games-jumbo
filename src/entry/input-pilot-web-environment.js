...
export function createInputPilotPageOwnerId(root = globalThis) {
  try {
    if (typeof root.crypto?.randomUUID === 'function') {
      return `pilot-page-${root.crypto.randomUUID()}`;
    }
    if (typeof root.crypto?.getRandomValues === 'function') {
      const values = new Uint32Array(4);
      root.crypto.getRandomValues(values);
      return `pilot-page-${[...values].map((value) => value.toString(16).padStart(8, '0')).join('')}`;
    }
  } catch {
  ...
  const monotonic = Number.isFinite(root.performance?.now?.())
    ...
    : 0;
  return `pilot-page-fallback-${wall}-${monotonic}`;
}
