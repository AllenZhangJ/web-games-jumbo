...
状态：首个正式纵切已实现，并保留程序化加载失败回退；Stage 6 E3/E4 与 Mapper 冻结仍阻断批量动作精修和 Stage 7 冻结。
...
- 使用 `arena.stage7.formal-asset-budget.v1` 复算 GLB/PNG/OGG 的文件、动作、节点、关节、primitive、材质和纹理解码内存；预算已进入构建前门禁，后续与目标真机数据共同进入 Stage 9 冻结候选。
