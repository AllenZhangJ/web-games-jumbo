...
const DEFINITION_KEYS: ReadonlySet<string> = new Set([
  'schemaVersion', 'id', 'description', 'metricSchemaVersion', 'candidate',
  'seedSet', 'workload', 'collectors', 'limits',
]);
const CANDIDATE_KEYS: ReadonlySet<string> = new Set([
  'id', 'sourceCommit', 'sourceDirty', 'matchConfig', 'authority',
]);
const AUTHORITY_KEYS: ReadonlySet<string> = new Set([
  'matchSchemaVersion', 'physicsBackendVersion', 'configHash', 'ruleContentHash',
]);
const SEED_SET_KEYS: ReadonlySet<string> = new Set(['kind', 'values', 'first', 'last']);
const WORKLOAD_KEYS: ReadonlySet<string> = new Set(['id', 'version', 'parameters']);
const COLLECTOR_KEYS_V1: ReadonlySet<string> = new Set(['id', 'version']);
const COLLECTOR_KEYS_V2: ReadonlySet<string> = new Set(['id', 'version', 'parameters']);
const LIMIT_KEYS: ReadonlySet<string> = new Set(['maximumTicksPerCase', 'maximumFailedCases']);
...
export interface ArenaExperimentAuthority {
  readonly matchSchemaVersion: number;
  readonly physicsBackendVersion: string;
  readonly configHash: string;
  readonly ruleContentHash: string;
}
export interface ArenaExperimentCandidate {
  readonly id: string;
  readonly sourceCommit: string;
  readonly sourceDirty: boolean;
  readonly matchConfig: Readonly<PlainRecord>;
  readonly authority: Readonly<ArenaExperimentAuthority>;
}
export type ArenaExperimentSeedSet =
  | { readonly kind: 'explicit'; readonly values: readonly number[] }
  | { readonly kind: 'range'; readonly first: number; readonly last: number };
export interface ArenaExperimentWorkloadReference {
  readonly id: string;
  readonly version: number;
  readonly parameters: Readonly<PlainRecord>;
}
export interface ArenaExperimentCollectorReference {
  readonly id: string;
  readonly version: number;
  readonly parameters?: Readonly<PlainRecord>;
}
export interface ArenaExperimentLimits {
  readonly maximumTicksPerCase: number;
  readonly maximumFailedCases: number;
}
export interface ArenaExperimentDefinitionData {
  readonly schemaVersion: 1 | 2;
  readonly id: string;
  readonly description: string;
  readonly metricSchemaVersion: number;
  readonly candidate: Readonly<ArenaExperimentCandidate>;
  readonly seedSet: ArenaExperimentSeedSet;
  readonly workload: Readonly<ArenaExperimentWorkloadReference>;
  readonly collectors: readonly Readonly<ArenaExperimentCollectorReference>[];
  readonly limits: Readonly<ArenaExperimentLimits>;
}
...
function identifier(value: unknown, name: string): string {
  ...
  if (!ID_PATTERN.test(id)) throw new TypeError(`${name} 只能包含小写字母、数字、点、下划线和连字符。`);
  return id;
}
...
function cloneAuthority(value: unknown): Readonly<ArenaExperimentAuthority> {
  ...
  return Object.freeze({
    ...
    physicsBackendVersion: boundedString(value.physicsBackendVersion, 128, `${name}.physicsBackendVersion`),
...
function cloneCandidate(value: unknown): Readonly<ArenaExperimentCandidate> {
  ...
  if (typeof value.sourceDirty !== 'boolean') throw new TypeError(`${name}.sourceDirty 必须是布尔值。`);
  const matchConfig = assertPlainRecord(
    cloneFrozenData(value.matchConfig, `${name}.matchConfig`),
    `${name}.matchConfig`,
  );
  return Object.freeze({
...
function cloneSeedSet(value: unknown): ArenaExperimentSeedSet {
  ...
  if (value.kind === ARENA_EXPERIMENT_SEED_SET_KIND.EXPLICIT) {
    ...
    return Object.freeze({ kind: ARENA_EXPERIMENT_SEED_SET_KIND.EXPLICIT, values: Object.freeze(values) });
  }
  if (value.kind === ARENA_EXPERIMENT_SEED_SET_KIND.RANGE) {
    ...
    if (last - first + 1 > MAXIMUM_CASES) throw new RangeError(`${name} 不能超过 ${MAXIMUM_CASES} 个 seed。`);
    return Object.freeze({ kind: ARENA_EXPERIMENT_SEED_SET_KIND.RANGE, first, last });
  }
  throw new RangeError(`${name}.kind 不受支持：${String(value.kind)}。`);
}
function cloneWorkload(value: unknown): Readonly<ArenaExperimentWorkloadReference> {
  const name = 'ArenaExperimentDefinition.workload';
  assertKnownKeys(value, WORKLOAD_KEYS, name);
  const parameters = assertPlainRecord(
    cloneFrozenData(value.parameters, `${name}.parameters`),
    `${name}.parameters`,
  );
  return Object.freeze({
    id: identifier(value.id, `${name}.id`),
...
function cloneCollectors(
  values: unknown,
  schemaVersion: 1 | 2,
): readonly Readonly<ArenaExperimentCollectorReference>[] {
  ...
  const collectors = values.map((value, index): Readonly<ArenaExperimentCollectorReference> => {
    const name = `ArenaExperimentDefinition.collectors[${index}]`;
    assertKnownKeys(
      value,
      schemaVersion === ARENA_EXPERIMENT_DEFINITION_LEGACY_SCHEMA_VERSION ? COLLECTOR_KEYS_V1 : COLLECTOR_KEYS_V2,
      name,
    );
    const id = identifier(value.id, `${name}.id`);
    if (ids.has(id)) throw new RangeError(`重复的实验 collector ${id}。`);
    ids.add(id);
    const version = assertIntegerAtLeast(value.version, 1, `${name}.version`);
    ...
    return Object.freeze({ id, version, parameters });
  });
  return Object.freeze(collectors.sort((left, right) => (
    left.id < right.id ? -1 : left.id > right.id ? 1 : 0
  )));
}
function cloneLimits(value: unknown): Readonly<ArenaExperimentLimits> {
  const name = 'ArenaExperimentDefinition.limits';
  assertKnownKeys(value, LIMIT_KEYS, name);
  return Object.freeze({
    maximumTicksPerCase: assertIntegerAtLeast(value.maximumTicksPerCase, 1, `${name}.maximumTicksPerCase`),
    maximumFailedCases: assertIntegerAtLeast(value.maximumFailedCases, 0, `${name}.maximumFailedCases`),
  });
}

export class ArenaExperimentDefinition implements ArenaExperimentDefinitionData {
  declare readonly schemaVersion: 1 | 2;
  declare readonly id: string;
  declare readonly description: string;
  declare readonly metricSchemaVersion: number;
  declare readonly candidate: Readonly<ArenaExperimentCandidate>;
  declare readonly seedSet: ArenaExperimentSeedSet;
  declare readonly workload: Readonly<ArenaExperimentWorkloadReference>;
  declare readonly collectors: readonly Readonly<ArenaExperimentCollectorReference>[];
  declare readonly limits: Readonly<ArenaExperimentLimits>;
  ...
  constructor(value: unknown) {
    ...
    Object.defineProperties(this, {
      schemaVersion: { value: source.schemaVersion, enumerable: true },
      id: { value: identifier(source.id, 'ArenaExperimentDefinition.id'), enumerable: true },
      description: { value: boundedString(source.description, 1_000, 'ArenaExperimentDefinition.description'), enumerable: true },
      metricSchemaVersion: {
        value: assertIntegerAtLeast(source.metricSchemaVersion, 1, 'ArenaExperimentDefinition.metricSchemaVersion'),
        enumerable: true,
      },
      ...
      collectors: { value: cloneCollectors(source.collectors, source.schemaVersion), enumerable: true },
      limits: { value: cloneLimits(source.limits), enumerable: true },
    });
    Object.freeze(this);
  }

  getSeeds(): readonly number[] {
    const seedSet = this.seedSet;
    if (seedSet.kind === 'explicit') return seedSet.values;
    return Object.freeze(Array.from(
      { length: seedSet.last - seedSet.first + 1 },
      (_, index) => seedSet.first + index,
    ));
  }
  toJSON(): ArenaExperimentDefinitionData {
    return {
...
export function createArenaExperimentDefinition(value: unknown): ArenaExperimentDefinition {
