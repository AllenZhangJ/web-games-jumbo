...
| 验证 | 直接规格已纳入SF-A3A6P.1固定runner；解码、灰度、截图、构建、浏览器、设备、性能和生命周期仍顺延 |
...
源码构造时会拒绝5纹理/5语义/483 Cue不闭合、path/bytes/SHA漂移、批准漂移、击落/坠落混因果、程序化Fallback、粒子或过度绘制扩容，以及默认入口消费；直接规格已由固定runner执行。十项真实评审仍`not-run`，不能声明纹理解码、灰度、可读性、GPU、过度绘制、性能、设备或生命周期通过。
