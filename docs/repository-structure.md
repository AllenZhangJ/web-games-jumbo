...
本文描述第 0 批基线的实际结构，不提前把治理目标写成现状。未来目录迁移后，本文件必须随对应批次校准。
...
| `package.json` | Node 版本、运行命令和直接依赖。 |
...
| 文件 | 当前职责 |
|---|---|
| `game-state.js` | 回合状态机、当前/目标值、步数、候选运算、蓄力阶段和胜负。当前直接读取全局 `GAME_RULES`。 |
| `operations.js` | 运算执行、候选生成、目标距离和受限深度求解器。 |
...
当前耦合：`game-state.js` 直接依赖根级配置；数值任务、难度和回合仍集中在一个类中。第二批会把 Gameplay、Task 和 Jump Engine 分开。

## `src/runtime`：当前应用编排

| 文件 | 当前职责 |
|---|---|
| `game.js` | `NumberStrategyGame`；同时负责平台输入、生命周期、固定步长、跳跃编排、落地事务、表现状态、错误恢复和 Renderer 调用。 |
...
| 文件 | 当前职责 |
|---|---|
| `src/config.js` | 设计尺寸、全局游戏规则、主题颜色和跳跃物理；当前混合了领域与表现配置。 |
...
当前测试总数为 89。项目尚未启用覆盖率统计，因此“有测试”不能等同于已经满足未来覆盖率门槛。
