...
该 strict TypeScript workspace 承接 Product 的宿主无关表现编排。当前公开能力包括 UI/Gameplay 输入路由、UI 意图串行化、内容/屏幕 Definition、只读 Registry、本地消息目录、Arena V1 静态内容工厂、只读 ViewModel、宿主无关 UI SceneModel 投影、单局表现桥、Product Presentation Flow 与注入式 Session 所有权根；上层只保留具体 Arena 内容、Renderer、Probe、平台和产品 Controller 的应用组合。
