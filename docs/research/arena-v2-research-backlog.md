...
当前新增证据：规则层横向冲量已经接入轻量物理，在宽平台、KZ 灰盒窄路和边缘平台形成不同的“命中但安全 / 命中后出界”结果，详见[武器地图边缘原型结果](arena-v2-weapon-map-prototype-results-v1.md)。下一步不是继续堆静态数值，而是加入移动目标、攻击者失位和多人拥挤，验证这些差异是否仍然可读且可反制。
