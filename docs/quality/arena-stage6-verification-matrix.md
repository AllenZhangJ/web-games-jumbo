...
执行中。本文定义 Stage 6 的完成证据，不表示后续批次已经通过。S6.1～S6.6.3c 已分批完成对应本机门禁；Web 真实浏览器和 100 局 Session soak 已通过。S6.6.3b/c 已在双槽 Workspace 上补齐 Trial Controller、终态表单/导出、可恢复复核草稿与独立 Web 采集入口；S6.6.4 已建立版本化 E3 Definition、构建绑定、证据包和附件完整性验证器，但还没有任何目标设备 Record。真实新手 E4 样本、微信/抖音开发者工具和目标真机 E3 仍未通过。实现边界见 [Stage 6 输入、移动与灰盒执行计划](../architecture/arena-stage6-input-movement-plan.md)，决策背景见 [ADR-009](../decisions/009-arena-semantic-input-and-movement-authority.md)、[ADR-013](../decisions/013-arena-pilot-local-evidence-workspace.md) 与 [ADR-014](../decisions/014-arena-versioned-device-acceptance-evidence.md)。
...
S6.6.4 已建立独立于 Rule/Core、Session 和 Pilot Workspace 的设备验收合同：固定 Web 手机、微信/抖音开发者工具与真机五个 target；每条 Record 绑定完整 commit/build、客户端/设备和全部必测项；附件以相对路径、字节数与 SHA-256 校验，CLI 额外拒绝符号链接逃逸。同一构建只要存在失败 Record，后续成功运行也不能抵消。自动化结果见 [设备证据合同门禁记录](../research/arena-stage6-device-evidence-contract.md)，执行步骤见 [E3/E4 验收操作手册](../acceptance/stage6/README.md)；该基础只让 E3 可验证，不等于已有 E3 证据。
