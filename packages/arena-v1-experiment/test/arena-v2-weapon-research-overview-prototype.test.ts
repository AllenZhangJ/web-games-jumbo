...
describe('Arena V2 weapon research overview matrix', () => {
  it('shows comparable numeric axes, units and direction semantics for all three candidates', () => {
    ...
    expect(matrix.rows.every(({ stats, behaviorStats, coreVerb, hitResult, mapSpaces, counterplay }) => (
      stats.length === 9
    ...
    expect(matrix.rows[0]?.stats.find(({ id }) => id === 'range')?.value)
      .toBeGreaterThan(matrix.rows[1]?.stats.find(({ id }) => id === 'range')?.value ?? 0);
    expect(matrix.rows[1]?.stats.find(({ id }) => id === 'startup')?.direction)
      .toBe('lower-is-better');
    expect(matrix.rows[0]?.behaviorStats.find(({ id }) => id === 'direction-tolerance')?.unit)
      .toBe('°');
  });
  ...
  it('freezes the research-only matrix and preserves distinct behavior fingerprints', () => {
    ...
    expect(Object.isFrozen(first.rows[0]?.stats)).toBe(true);
  });
});
