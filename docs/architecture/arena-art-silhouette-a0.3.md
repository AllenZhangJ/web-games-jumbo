...
检查器验证正式资产路径/hash、clip、骨架、方向/距离/视口全组合、RGB/alpha/二值像素、尺寸、匿名题包、答案泄露、真人样本、90%阈值和下游状态。14个负向探针覆盖缺资产、hash漂移、兜底路径、clip/骨架错误、方向/距离缺失、背景、尺寸、alpha、答案泄露、样本不足、阈值降低和Blockout越级。

下一步只允许由至少10名未参与制作的真人完成匿名测试，并复算角色/装备/方向混淆矩阵及距离分层；任一总指标<90%或任一角色×装备组合<80%仍需退回姿态/体块/持握返工。不得加颜色、文字、描边、发光，不得修改权威hitbox或玩法。
