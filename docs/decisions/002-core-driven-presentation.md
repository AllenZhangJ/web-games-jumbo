...
1. `src/core` 是数值、世界、角色绝对位置、跳跃轨迹和碰撞结果的唯一真相。
2. Runtime 是唯一编排者：它处理输入、以固定步长更新 Core，然后把只读快照传给 Renderer。
3. `Renderer3D.draw(state, world, presentation)` 只消费快照与表现事件，不持有可回写的 Core 对象。
...
Runtime fixed-step orchestration
      ↓
Core state + collision transaction
      ↓
