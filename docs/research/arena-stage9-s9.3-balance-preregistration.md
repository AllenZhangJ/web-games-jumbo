...
三件装备均有充分样本，但淘汰作用不对称：锁链 18 次、大锤 1,735 次、盾牌 1,226 次归因淘汰。该数字不是当前独立阻断门。S9.3b 第一轮先只改变生命数，验证完整地图时间轴是否自然改变淘汰结构；若隔离 validation 仍失败，再用新的预注册候选处理装备击飞、锁链战术价值或地图机会，避免首轮同时改动多个系统。
