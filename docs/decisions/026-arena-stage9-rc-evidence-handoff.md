...
- 状态：已接受；S9.6a 交接合同与 S9.6b1-b5b 十一个 producer 已实施，正式资产 Intake/批准已就绪，其 producer 与目标真机证据待完成
- 日期：2026-07-18
...
S9.6b5a 增加 `arena:defects:verify`。`ArenaDefectLedger` 不包含可手写状态；Report 从 `knownIssuesComplete`、开放严重度、解决证据和 residual risk 关系派生。每个开放缺陷必须由具名风险 owner、缓解措施与复核触发条件承接；已解决缺陷必须保留 resolution summary 与 verification references；开放 blocking/high 直接失败，未完成已知问题复核保持 incomplete。Ledger 绑定候选 commit，source producer 继续要求 clean checkout 前后稳定。

S9.6b5b 增加 `arena:input-pilot:evidence`。工作台只允许 clean、包含 `pilot.html` 的 Web Manifest 形成正式 Evidence Bundle；持久化 key 同时包含 Definition、commit、buildId 和 Manifest hash，避免旧构建记录混入新候选。Bundle 保留完整 pseudonymous Audit，producer 重新验证固定 Definition、逐条 Record、派生 Report 和 `candidate-winner`，再复验候选中的 Stage 6 Device Gate；缺少 E3、样本不足、阈值失败、build 漂移或手写结论均不能 ready。CLI 与 release 复验共享同一 verifier，Web Manifest 还必须与最终构建门一致。

Formal Assets 继续保持未验证。它必须等待 S7.2～S7.5 真实资产 Definition、预算、许可与 reduced-motion 证据，不能用 S7.1 程序化占位填充。
...
- 当前十一个 Gate 具备 release producer 语义适配，仍不能形成 ready 候选。只有正式资产 producer 和全部真实门完成后才能关闭本 ADR 的实施状态。
