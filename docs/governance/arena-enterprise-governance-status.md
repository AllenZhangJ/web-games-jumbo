...
1. 当前 362 个受维护 JavaScript 文件仍在精确允许清单中，Presentation Session、Platform 和 Arena V1 应用注入适配尚未完成 strict TypeScript workspace 迁移。
