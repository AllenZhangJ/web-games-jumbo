...
第一轮结果已记录在[热血英豪武器研究 V1](arena-v2-hot-blooded-weapon-study-v1.md)。当前结论是先建立 7 个可比较的公开数值：攻击距离、出手速度、收招时间、横向击退、垂直控制、自身位移和冷却时间。
...
第一轮结果已记录在 [CS1.6 KZ 跳跃地图研究 V1](arena-v2-cs16-kz-map-study-v1.md)。当前结论是：使用段落化路线和难度轴，竞速与生存共用地图基座，但生存不设置终点。
...
第一轮结果已记录在 [弹壳特攻队界面研究 V1](arena-v2-survivor-io-ui-study-v1.md)。当前结论是：先把武器索引放入主页验证阅读行为，再决定是否拆独立武器库页面。
