...
const score = {
  ...
  deterministicRenderCoverage: { earned: inFrame === 144 ? 20 : 16, possible: 20, evidence: `144个组合均生成，但只有${inFrame}/144在固定target相机内有像素；12m共48项离屏。` },
...
const gate = {
  ...
  facts: { expectedOutputs: 144, actualOutputs: outputs.length, inFrameOutputs: inFrame, offscreenOutputs: outputs.length - inFrame, humanParticipants: human.participantCount, requiredHumanParticipants: human.minimum, proxy: proxyAggregate },
  ...
  blockers: [
    ...(inFrame === 144 ? [] : [`固定target镜头仍有${outputs.length - inFrame}个离屏空样本。`]),
