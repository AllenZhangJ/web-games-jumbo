...
> **项目状态：** 当前可视产品仍是数值跳台 v3。Arena V1 已完成独立轻量物理、无渲染 1v1 MatchCore、隐藏本地机器人、三件装备和 Stage 5 地图权威时间轴；Arena 可视渲染与触控灰盒尚未接入。两条领域代码保持隔离。
...
- [ADR-010：Arena 使用语义表现合同与独立资产注册表（提议）](docs/decisions/010-arena-semantic-presentation-and-assets.md)
- [ADR-011：Arena 使用版本化双槽本地进度与对称内容池（提议）](docs/decisions/011-arena-versioned-local-progression.md)
