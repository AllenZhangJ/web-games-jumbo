...
- 输入：已签核A0.1、六类文字登记、可嵌入素材的权利/proof/hash、受限来源link-only清单。
- 输出：六份`2560×1440` SVG+PNG、每板10格7/2/1布局、六份manifest、全部输入hash、美术指导与协调签核。
...
- 硬门：总分≥90且每维度≥80%；每板至少6个合法嵌入视觉，不能空占位或全link-only；六板双签核。当前`incomplete`，不评分。
