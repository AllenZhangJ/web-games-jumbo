export type SynchronousCompositionMethod = (
...
if (CAPTURED_THEN_DESCRIPTOR === undefined
  ...
  throw new TypeError('Product Composition无法捕获原生Promise.prototype.then。');
}
...
if (CAPTURED_SPECIES_DESCRIPTOR === undefined
  ...
  throw new TypeError('Product Composition无法捕获原生Promise[Symbol.species]。');
}
...
function assertNativePromiseIntegrity(): void {
  ...
  if (thenDescriptor === undefined
    ...
    throw new TypeError('Product Composition原生Promise.prototype.then描述符漂移。');
  }
}
...
function assertNativePromiseSpeciesIntegrity(): void {
  ...
  if (descriptor === undefined
    ...
    throw new TypeError('Product Composition原生Promise[Symbol.species]描述符漂移。');
  }
}

export function captureSynchronousCompositionMethod(
  value: unknown,
  methodName: string,
  ownerName: string,
): SynchronousCompositionMethod {
  ...
  for (let depth = 0; cursor !== null && depth < MAX_PROTOTYPE_DEPTH; depth += 1) {
    ...
    if (descriptor !== undefined) {
      ...
      const method = descriptor.value as SynchronousCompositionMethod;
      return (...arguments_: readonly unknown[]) => Reflect.apply(method, value, arguments_);
    }
...
export function assertSynchronousCompositionResult<T>(value: T, label: string): T {
