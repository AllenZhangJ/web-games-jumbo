...
- 首批代码提交：`c523d7a00e78a0342e1f2bbe1ef65e12fbb8888d`
- 完整基础证据提交：`e56ab2f992c492de163d96792716e36a9d472b28`
...
新增 strict `@number-strategy-jump/arena-device-acceptance`，拥有：
...
- 设备 Record、Bundle、派生 Report 与三端 Build Manifest 数据合同。

该包只允许依赖 `arena-contracts` 与标量层 `arena-evidence-contracts`，不得依赖 Presentation、Performance、Product、Release、Node、Three.js、DOM、平台 API、墙钟、随机源或网络。所有数据先完整复制、校验并冻结；访问器、Symbol、循环引用、非有限值、重复 id、悬空 check、空附件集合和非法枚举在公开数据发布前失败关闭。
...
代价：迁移期同时存在 strict 设备证据包与上层 Stage 9 性能实现；新增性能目标必须在上层组合中完成双向注册校验，不能直接加入低层设备包。
