...
> **项目状态：** Web、微信、抖音默认入口已切换到 Arena V1 Stage 6 灰盒：独立轻量物理、1v1 MatchCore、隐藏本地机器人、三件装备、地图时间轴、语义移动/触控、程序化角色、HUD、结算与重赛已经连成可玩闭环。Stage 8 S8.1～S8.4 的无 UI 产品基础也已落地，包括可靠本地 Profile、幂等奖励、双方共享冻结内容池、Replay V5 与快捷重赛。当前已通过 Web 真实浏览器和 100 局 Presentation Session soak；微信/抖音开发者工具、目标真机 E3、A/B 新手盲测和 S8.5 三端产品 UI 仍未完成。数值跳台 v3 代码与资产继续保留，两条领域代码保持隔离。
...
- [Arena Stage8 S8.5.1 产品表现合同基础](docs/research/arena-stage8-product-presentation-foundation.md)
...
- [ADR-012：Arena 使用可复现实验收敛并只降级表现层（提议）](docs/decisions/012-arena-reproducible-convergence.md)
