...
2026-07-22 的治理实现把构建预算 Policy/Report 及性能 Policy/Record 迁入 strict `@number-strategy-jump/arena-performance-evidence`。该包只依赖不可变数据、证据标量与设备验收合同，只重算调用方注入的 Manifest/Trace；架构门禁止它读取宿主计时、DOM、Three.js、平台 API、随机源或网络。运行时采集继续由 `arena-presentation-runtime` 的注入式观察器负责，证据包不持有其生命周期。性能 Metric/Report 与 Stage 9 组合仍待按同一边界迁移，因此这次实现更新不等于 S9.4 已关闭。
