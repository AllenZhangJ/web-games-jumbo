...
- 状态：研究工具链原型通过；魔血镰刃、白金双枪、血影钩刃、幻虎巨拳与真·哈迪斯钩镰已接入研究 Definition 数值投影，仅猛犸石斧仍待候选 Definition
- 日期：2026-07-28
...
当前生产武器概览已经能够从权威 `Definition` 投影 9 个主数值轴、2 个行为轴和地面/空中上下文。六件逐动作深研案例中，白金双枪已经映射到 `research-line-pressure`，魔血镰刃、血影钩刃、幻虎巨拳和真·哈迪斯钩镰已经有独立研究 Definition 与 Replay；魔血镰刃、幻虎巨拳和真·哈迪斯钩镰另有边缘结果，血影钩刃另有独立障碍探针；猛犸石斧仍只有官方事实、设计推导、反制、失败成本和数值审计，不能填入假数值。
...
魔血镰刃的宽覆盖、active 前离开攻击线和平台边缘击落已经由真实 MatchCore/MatchReplay 验证；延迟危险区仍保持独立研究假设。血影钩刃的方向分支已经由 `rear-cone` 与目标主动转身 Replay 验证；障碍阻挡仍由独立二维探针验证，不会被伪装成现有 9 个主比较轴。真·哈迪斯钩镰已经由真实 MatchCore/MatchReplay 验证提前释放、到期取消、提交后命中和平台边缘击落，但这只证明“承诺与支撑面后果”可以被权威链路读取，不等同于原作的完整反弹、多阶段追击或正式平衡。障碍依赖仍需接入真实地图表面、空中高度和复活重新进入路线后，才有资格讨论是否新增玩家可读字段。
...
- 断言覆盖六件案例顺序、五件研究投影的来源 Definition 与上下文、`must-measure`/`research-only` 差异、血影钩刃障碍信号、魔血镰刃路线后果、目标朝向分支、确定性和深冻结；
- 当前生产边界不变：研究 Definition 未注册默认生产装备。

## 下一步

1. 为猛犸石斧建立最小候选 Definition，并把魔血镰刃的 `warningHypothesis` 接入独立预警运行时；
2. 对每件候选的地面/空中动作测出主 9 轴和行为 2 轴，不能凭官方原作数值换算；
3. 将五件已有投影统一接入可读性矩阵，再进行真人首见时间、命中归因和反制可读性测试；
