...
执行中，2026-07-18。S9.1a 已落地版本化 Experiment Definition、Workload/Collector Registry、无宿主 Simulation Runner、确定性 Report 与第一条 Arena V1 脚本压力 workload；S9.1b 已把 MatchCore 状态不变量、事件上限、seed 隔离和抽样回放迁入同一编排，并让 CPU/heap 门继续直接驱动同一 workload case。Map/Bot/Movement 专业压测尚未迁移，S9.2～S9.5 仍未开始。Stage 8 已提供版本化设备证据和构建 Manifest 边界；Stage 9 性能指标 Definition、精确设备与预算仍需在真机门禁前由项目方确认。
...
- 继续迁移 Map/Bot/Movement 专业压测，保留各自专业断言。
