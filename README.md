...
Stage 9 RC 交接门使用 `npm run arena:stage9:readiness -- --describe` 查看。S9.6b1-b5a 已启用十个 producer：构建两门复用同一组三端 Manifest 并重新遍历产物；黄金回放、平衡验证与原子组合回归各自重算；Stage 6/8 设备、性能和真人研究复用原 CLI 的共享 verifier；缺陷门从版本化账本派生开放严重度和残余风险结论。外部材料即使合同合法，样本不足仍只能得到 `incomplete`。source producer 还要求当前 clean checkout 与候选 commit 完全一致且复验前后身份稳定。输入盲测与正式资产尚未适配，候选 JSON 中未经复算的 `ready` 仍按 incomplete 处理；完整缺口见 [Stage 4～9 证据矩阵](docs/quality/arena-stage4-9-evidence-matrix.md)，账本步骤见 [S9.6 缺陷与风险账本手册](docs/acceptance/stage9-release/README.md)。
