...
调研记录，更新于 2026-07-21。架构候选只借鉴设计思想；Three.js 官方能力已通过项目现有依赖用于正式 GLTF/动画运行时，KayKit 与 Kenney 资产另按固定 revision、许可和字节摘要入库。没有从其余候选复制代码；若后续实质复用，必须固定 commit、核对许可并更新 `THIRD_PARTY_NOTICES.md`。
...
项目当前依赖 Three.js `0.185.1`，本轮固定研究 commit `2431a09f46f34c560bc8e44b33be0e567723d5b9`、MIT。Stage 7 已使用官方 `GLTFLoader`、`AnimationMixer` 和 `SkeletonUtils` 接入双 KayKit 正式角色、骨骼动画与克隆运行时；当前 Gameplay V2 进一步使用上半身 track mask 保持 locomotion 与攻击叠加。实现和许可边界详见 [手机动作战斗专项 GitHub 调研](arena-mobile-action-combat-github-study.md) 与 [第三方声明](../../THIRD_PARTY_NOTICES.md)。
