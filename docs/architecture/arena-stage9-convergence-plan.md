...
执行中，2026-07-18。S9.1a～S9.1c 实验与专业 workload 已落地，S9.2 已建立版本化 Replay V5 黄金语料、严格历史拒绝、input fuzz 单例复现与生命周期回归门。S9.3 已保留首份 900 局失败 baseline，完成 540 局隔离探索与另一组 900 局 validation，并通过共享不可变 Balance Definition 将 11 条命提升为 Product 默认；MatchCore 通用默认与显式测试/模式覆盖保持独立。S9.4a 已建立质量 Definition、只读有界 Probe、性能 Policy/Record/Report、六目标设备证据和三端构建预算；六个真实 target Record 尚未采集，因此 S9.4 未冻结。S9.5a 已建立预注册真人研究 Definition、隐藏 block 分组、生产 seed、Product 完成端口、Capture Session、Record/Bundle/Report 和逐 Tick Replay/Bot 复验 CLI；独立可恢复采集工作台与真实样本尚未完成，因此 S9.5 未冻结。
...
- S9.5b 待完成独立、可恢复、去标识采集工作台，再招募至少 90 名合格完成者并运行最终 CLI。
- 汇总配置 hash、报告、回放、缺陷清单、三端证据和剩余风险；通过后形成 Stage 10 RC 输入，未通过则只修复当前候选。
...
低档表现 30 FPS/Core 60 Hz、启动/帧时间/内存/包体 V1 默认预算和 S9.5 V1 研究阈值均已进入版本化合同，不再是当前未决项。外部设备、独立采集工作台和真人证据会阻塞 S9.4/S9.5 冻结，但不阻塞工程基础继续完善。
