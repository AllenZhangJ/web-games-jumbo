...
function cloneExpectedEventCounts(value: unknown): Readonly<Record<string, number>> {
  const source = assertPlainRecord(
    cloneFrozenData(value, 'map timeline expectedEventCounts'),
    'map timeline expectedEventCounts',
  );
...
function sortedCounts(values: ReadonlyMap<string, number>): Readonly<Record<string, number>> {
  return Object.freeze(Object.fromEntries([...values.entries()].sort(([left], [right]) => (
    left < right ? -1 : left > right ? 1 : 0
  ))));
}

export function createArenaV1MapTimelineParameters(value: unknown) {
  ...
  return Object.freeze({
    ...
    expectedEnabledSurfaceIds: cloneFrozenStringSet(
      source.expectedEnabledSurfaceIds as readonly unknown[],
...
class ArenaV1MapTimelineCase {
  ...
  #parameters: ReturnType<typeof createArenaV1MapTimelineParameters> | null;
  ...
  constructor({ seed, candidate, parameters }: ArenaSimulationCaseFactoryOptions) {
    ...
    try {
      ...
      try {
        ...
        throw Object.assign(new Error('Map timeline case 构造失败且 Core 清理失败。'), {
          originalError: error,
          cleanupError,
        });
      }
  ...
  #requireCore() {
    this.#assertUsable();
    if (!this.#core) throw new Error('ArenaV1MapTimelineCase 缺少 Core。');
    return this.#core;
  }

  #requireParameters() {
    this.#assertUsable();
    if (!this.#parameters) throw new Error('ArenaV1MapTimelineCase 缺少参数。');
  ...
  step() {
    ...
    const frames = core.config.participantIds.map((participantId) => (
    ...
    if (this.#eventCount > parameters.maximumEventsPerCase) {
      throw new Error(
        `Map timeline case ${core.matchSeed} 事件数超过`
  ...
  exportResult() {
    ...
    const finalState = assertArenaMapTimelineFinalState(finalSnapshot, parameters);
    const eventCounts = sortedCounts(this.#eventCounts);
    for (const [type, expected] of Object.entries(parameters.expectedEventCounts)) {
      if ((eventCounts[type] ?? 0) !== expected) {
        throw new Error(
          `Map timeline case ${core.matchSeed} ${type} 数量`
    ...
    if (this.#runner) {
      ...
      if (replayed.finalHash !== replay.finalHash || replay.finalHash !== finalHash) {
        throw new Error(`Map timeline case ${core.matchSeed} 回放 hash 不同。`);
      }
      replayVerified = true;
    }
    return Object.freeze({
  ...
  destroy() {
    ...
    if (errors.length > 0) {
      throw Object.assign(new Error('ArenaV1MapTimelineCase 清理未完整完成。'), {
...
export function createArenaV1MapTimelineWorkloadEntry() {
  return Object.freeze({
    ...
    createCase: (options: ArenaSimulationCaseFactoryOptions) => new ArenaV1MapTimelineCase(options),
  });
}
