...
| G7 零 JS/完整质量门 | 进行中 | 独立 strict `arena-human-match-study` 已承接真人研究通用 Definition、Assignment、Capture Session、Submission/Record、Bundle、Capture Package、Checkpoint/Receipt/Workspace/Envelope/Repository/Controller、评估 Report，以及 Arena Stage 9 真人公平性 V1 内容定义；独立 strict `arena-human-match-study-verification` 承接与 Arena V1 Product/Bot 组合绑定的 Replay 复验；独立 strict `arena-release-contracts` 承接发布就绪 Definition 与 Evidence Statement，独立上层 strict `arena-release` 承接候选 Bundle、producer 结果复验、就绪 Report、缺陷账本/报告，以及构建、设备、真人公平性、Input Pilot 和缺陷发布证据；独立 strict `arena-input-pilot` 已承接 Input Pilot 共享词汇、Definition/Registry/Assignment/V1、Record Fields/Record、Review Draft、Form Model、同步存储 Port/Lease、Assignment Match Service、Trial Runtime Port、Trial Checkpoint/State、Enrollment Ledger、Workspace/Envelope/Repository、Coordinator、评估 Report、Audit/Aggregate Export、Evidence Bundle、Action Metrics、Metric Collector、Observed Session/Match Service 与 Trial Controller；独立上层 strict `arena-input-pilot-presentation` 已承接 Pilot 表现运行时。ESLint、strict TypeScript、Vitest 和 JavaScript 精确递减门禁持续运行。其余 Release/实验/回归/测试链迁移、正式 coverage 阈值和零 JS 尚未完成 |
...
1. 当前 199 个受维护 JavaScript 文件仍在精确允许清单中；G7 尚需迁移其余 Release/实验/回归/测试与验收链并建立零 JS 门禁。
...
## G7.23 真人研究 Workspace Controller strict 迁移证据

- Workspace Controller 已迁入 strict `@number-strategy-jump/arena-human-match-study`，精确允许清单由 215 降至 214；Study Web App 与 Workspace 测试统一消费包公开 API，旧 JavaScript 真值已删除，没有兼容转发层，也没有新增或升级依赖。
- Controller 继续作为入组、开始、进度、复核、采集包、导出待确认、恢复和归档转换的唯一调用入口；所有转换仍通过规范 Workspace 构造、revision CAS 和 Repository 快照完成，不直接写存储。Repository 能力在构造时以数据描述符解析并绑定，拒绝方法访问器和缺失合同；Controller options、enrollment、review、capture、package receipt 与 CAS 回执均在读取业务字段前执行无访问器克隆或描述符校验。
- 生命周期扩为 `created/opening/open/failed/destroyed`。打开回调重入、提交回调重入和活动操作期间销毁均被拒绝；打开后的自动恢复、CAS 返回冲突/畸形回执、续租失败或快照重建失败会立即 fail closed。销毁失败不会提前清空 Definition/Repository，Controller 保留 failed 状态和同一资源所有权，底层故障修正后可重试 destroy；只有确认成功后才进入幂等 destroyed。
- Workspace 定向 Node 回归 10/10、完整 Node 698/698、strict package/治理测试 360/360 通过；ESLint、strict typecheck、JS 递减、产品依赖、Three 边界、本地生产依赖树、`git diff --check`、生产构建、Manifest、预算和生产产物边界均通过。新增回归覆盖 Controller 打开重入、enrollment 访问器零执行、CAS 冲突后失败关闭和销毁失败重试。
- 代码提交为 `15d31ef7923cde30b1dacab50f3239e8f79e6109`，clean build ID 为 `arena-15d31ef7923c-product`；Web/微信/抖音 delivery 为 `3807531 / 3835130 / 3835105 B`，JavaScript 为 `1463921 / 1507034 / 1507034 B`。三端 `sourceDirty=false`、默认入口均为 Product、Manifest 校验、预算和生产产物边界通过且 `freezeEligible=true`；交付字节与 G7.22 完全一致，本研究控制器未进入默认 Product bundle。联网生产依赖审计仍保持 G7.11 已登记的 G8 外部门禁状态。
- 本批没有改变研究业务允许的正常转换、研究页面、游戏玩法、Bot、Replay/Profile schema 或权威运行结果，不改变 Gameplay V2 配置 hash `8c322912`、任意距离挥空、攻击/命中/击退、动作/武器、移动/跳跃、画质、关节或正式资产，因此未新增浏览器/手机通过记录，也未重跑无关的黄金 Replay、输入 fuzz、专项生命周期或 soak。Study Report/Replay Verifier/Release 链及两个研究 Web App 仍待 strict 迁移；G7-G10 尚未完成，当前不可合并。

## G7.24 真人研究评估 Report strict 迁移证据

- 真人研究评估 Report 已迁入 strict `@number-strategy-jump/arena-human-match-study`，精确允许清单由 214 降至 213；证据 CLI/验证器、Release evidence 和测试统一消费包公开 API，旧 JavaScript 真值已删除且没有兼容转发层。本批没有新增或升级依赖。
- 报告继续从规范 Record 重建 arm 与总体分母，区分协议排除、作废、可分析和完成样本，并计算 session 胜率、95% Wilson 区间、目标时长占比、Bot 猜测率、公平/自然评分和再战率。样本充足性优先决定 `incomplete`，其余可用门禁再决定 `failed/ready`；极端难度差和相邻难度倒挂仍按预注册 botStrengthRank 判定，没有以展示顺序替代实验顺序。
- Report、Arm Summary、Aggregate、Gate、Wilson Interval 和状态获得只读 strict 类型；空 arm、缺失 completed selfReport 和不完整排序结果显式 fail closed。输入在逐项业务读取前整体执行无访问器克隆，数组项访问器读取次数为零；超过 10,000 条的输入在深克隆前拒绝，避免异常证据消耗无界内存。唯一 record/participant/assignment/enrollment/seed/artifact 身份、连续 enrollmentIndex、同 commit/build、sourceDataHash 与 resultHash 保持不变。
- 真人研究/CLI 定向 Node 回归 11/11、完整 Node 698/698、strict package/治理测试 361/361 通过；ESLint、strict typecheck、JS 递减、产品依赖、Three 边界、本地生产依赖树、`git diff --check`、生产构建、Manifest、预算和生产产物边界均通过。新增 strict 回归证明 Report 数组项访问器零执行。
- 代码提交为 `064f172bb0938dc98e2efeb1e4b5bf18a7f58782`，clean build ID 为 `arena-064f172bb093-product`；Web/微信/抖音 delivery 为 `3807531 / 3835130 / 3835105 B`，JavaScript 为 `1463921 / 1507034 / 1507034 B`。三端 `sourceDirty=false`、默认入口均为 Product、Manifest 校验、预算和生产产物边界通过且 `freezeEligible=true`；交付字节与 G7.23 完全一致，本离线报告未进入默认 Product bundle。联网生产依赖审计仍保持 G7.11 已登记的 G8 外部门禁状态。
- 本批没有改变研究数据口径、预注册阈值、页面、游戏玩法、Bot、Replay/Profile schema 或权威运行结果，不改变 Gameplay V2 配置 hash `8c322912`、任意距离挥空、攻击/命中/击退、动作/武器、移动/跳跃、画质、关节或正式资产，因此未新增浏览器/手机通过记录，也未重跑无关的黄金 Replay、输入 fuzz、专项生命周期或 soak。Study Replay Verifier/Release 链及两个研究 Web App 仍待 strict 迁移；G7-G10 尚未完成，当前不可合并。

## G7.25 真人研究 Replay Verifier strict 拆分证据

- 新增上层 strict `@number-strategy-jump/arena-human-match-study-verification`，承接与 Arena V1 MatchCore、正式 Balance、Bot Profile、Matchmaking 和 Product Result 绑定的 Replay 复验；精确允许清单由 213 降至 212。通用 `arena-human-match-study` 没有反向依赖 Product 组合层，证据验证器、ingest CLI 与测试统一消费新包，旧 JavaScript 真值已删除且没有兼容转发层。
- Verifier 先通过 `validateArenaReplay` 完整验证 schema、配置、帧、checkpoint、事件和结果，再核对预注册 candidate、天然隐藏难度、冻结 Product config hash、Record seed/result。复验时仍在每个 tick 以相同 behavior/personality seed 重新生成 player-2 Bot 输入并逐帧比较，最终同时核对 Replay finalHash、Product authorityHash 和 endedAtTick。
- options 使用数据描述符读取，访问器读取次数为零；Replay 不再先以弱克隆结果驱动 MatchCore。Probe MatchCore 在 BotController 构造前完成参数提取和销毁，避免 probe 清理失败时泄漏已创建 Bot；主复验失败与 Bot 清理失败会同时保留，Bot destroy 幂等。返回的验证身份和 hash 具有只读 strict 类型。
- 真人研究/CLI 定向 Node 回归 11/11、完整 Node 698/698、strict package/治理测试 362/362（57 个测试文件）通过；ESLint、strict typecheck、JS 递减、产品依赖、Three 边界、本地生产依赖树、`git diff --check`、生产构建、Manifest、预算和生产产物边界均通过。新增 strict 回归证明 Verifier options 访问器零执行。
- 代码提交为 `e055f7d26caf97925b39f535fab7a04870bf1815`，clean build ID 为 `arena-e055f7d26caf-product`；Web/微信/抖音 delivery 为 `3807531 / 3835130 / 3835105 B`，JavaScript 为 `1463921 / 1507034 / 1507034 B`。三端 `sourceDirty=false`、默认入口均为 Product、Manifest 校验、预算和生产产物边界通过且 `freezeEligible=true`；交付字节与 G7.24 完全一致，本离线验证包未进入默认 Product bundle。
- 本批只新增仓内 workspace 依赖，没有新增或升级外部包；但 `npm install --ignore-scripts` 在建立 workspace 链接时自动提示当前锁文件存在 3 个 high 依赖风险。未执行 `audit fix --force`、未自动升级或掩盖风险，该提示已登记为 G8 阻断，后续必须完成受控依赖审计、可达性判断和兼容升级验证后才能关闭。联网审计状态不再沿用此前“未观察到风险”的阶段性结论。
- 本批没有改变研究数据、报告口径、页面、游戏玩法、Bot 算法、Replay/Profile schema 或权威运行结果，不改变 Gameplay V2 配置 hash `8c322912`、任意距离挥空、攻击/命中/击退、动作/武器、移动/跳跃、画质、关节或正式资产，因此未新增浏览器/手机通过记录，也未重跑无关的黄金 Replay、输入 fuzz、专项生命周期或 soak。Study Release 链及两个研究 Web App 仍待 strict 迁移；G7-G10 尚未完成，当前不可合并。

## G7.26 发布就绪 Definition/Evidence Statement strict 合同证据

- 新增最底层 strict `@number-strategy-jump/arena-release-contracts`，承接 Release Readiness Definition、Gate、Evidence Statement 与 Material 合同；精确允许清单由 212 降至 210。全部 Release producer、candidate/report/verifier、Stage 9 handoff、Input Pilot CLI 和四组发布测试统一消费包公开 API，旧 JavaScript 真值已删除且没有兼容转发层。
- Definition 继续限制 S4～S10 阶段、最多 64 个唯一 gate、稳定 producer、source/build scope 与 8 位 requirement hash；Evidence Statement 继续绑定 definition gate、scope、40 位 commit、buildId、ready/failed/incomplete、结果 hash，以及最多 128 项唯一相对路径/sha256/受限字节材料。构造输入在业务读取前无访问器克隆，输出与 JSON 视图具有只读 strict 类型。
- 新包只依赖 `arena-contracts` 与 `arena-evidence-contracts`，producer、candidate、report 和 handoff 只能向下依赖合同层；根 workspace 构建、typecheck、prepare 和 prebuild 通过标准 `postbuild:packages` 生命周期包含该包，干净环境不会漏编译。安装使用 `--no-audit`，没有重复触发或掩盖 G7.25 已登记的 3 个 high 风险。
- 发布定向 Node 回归 25/25、完整 Node 698/698、strict package/治理测试 363/363（58 个测试文件）通过；ESLint、strict typecheck、JS 递减、产品依赖、Three 边界、本地生产依赖树、`git diff --check`、生产构建、Manifest、预算和生产产物边界均通过。新增 strict 回归证明 Definition 与 Statement 访问器零执行。
- 代码提交为 `ad112e8d79677b2922c407fee338e2f1ac7810bb`，clean build ID 为 `arena-ad112e8d7967-product`；Web/微信/抖音 delivery 为 `3807531 / 3835130 / 3835105 B`，JavaScript 为 `1463921 / 1507034 / 1507034 B`。三端 `sourceDirty=false`、默认入口均为 Product、Manifest 校验、预算和生产产物边界通过且 `freezeEligible=true`；交付字节与 G7.25 完全一致，发布合同未进入默认 Product bundle。
- 本批没有改变任何 gate requirement、发布状态判定、研究数据、页面、游戏玩法、Bot、Replay/Profile schema 或权威运行结果，不改变 Gameplay V2 配置 hash `8c322912`、任意距离挥空、攻击/命中/击退、动作/武器、移动/跳跃、画质、关节或正式资产，因此未新增浏览器/手机通过记录，也未重跑无关的黄金 Replay、输入 fuzz、专项生命周期或 soak。Release producer/bundle/report/verifier/handoff、两个研究 Web App 与其余 G7 链仍待 strict 迁移；G7-G10 尚未完成，当前不可合并。

## G7.27 发布候选 Bundle/复验/就绪 Report strict 迁移证据

- 新增上层 strict `@number-strategy-jump/arena-release`，承接 Release Candidate Bundle、producer 结果复验和 Readiness Report；精确允许清单由 210 降至 207。Stage 9 readiness/producers 与四组发布测试统一消费包公开 API，三份旧 JavaScript 真值已删除且没有兼容转发层。
- Candidate Bundle 绑定当前 Definition id/hash、40 位候选 commit、受限 buildId、`sourceDirty` 与 gate evidence；evidence 数量不得超过 gate 数，按 Definition gate 顺序稳定排序，重复 gate、commit/build 不一致和同路径材料 sha256/字节冲突均失败关闭。Producer 复验只接受已经属于当前 Candidate 且 content hash 相同的 Statement，并逐项核对 commit、buildId、status 与 resultHash。
- Readiness Report 只把已提供且 hash 复验通过的 evidence 纳入裁决；未验证或声明 incomplete 的 gate 保持 incomplete，失败 gate 或脏源候选不能成为 ready，只有全部 gate ready 才给出 `freezeEligible=true`。options 和输入均先做键集合/无访问器数据校验，新增 strict 回归证明 accessor 读取次数为零；所有输出深冻结并带确定性 result hash。
- 发布定向 Node 回归 25/25、完整 Node 698/698、strict package/治理测试 364/364（59 个测试文件）通过；ESLint、strict typecheck、JS 递减、产品依赖、Three 边界、本地生产依赖树、`git diff --check`、生产构建、Manifest、预算和生产产物边界均通过。新包只依赖仓内 `arena-contracts`、`arena-evidence-contracts` 与 `arena-release-contracts`，没有新增或升级外部依赖；G7.25 已登记的 3 个 high 依赖风险仍是 G8 阻断，未执行未经授权的联网审计或强制修复。
- 代码提交为 `1cafea66825982f71d394473768f7432c934e41d`，clean build ID 为 `arena-1cafea668259-product`；Web/微信/抖音 delivery 为 `3807531 / 3835130 / 3835105 B`，JavaScript 为 `1463921 / 1507034 / 1507034 B`。三端 `sourceDirty=false`、默认入口均为 Product、Manifest 校验、预算和生产产物边界通过且 `freezeEligible=true`；交付字节与 G7.26 完全一致，发布裁决包未进入默认 Product bundle。
- 本批没有改变 gate requirement、producer 业务算法、研究数据、页面、游戏玩法、Bot、Replay/Profile schema 或权威运行结果，不改变 Gameplay V2 配置 hash `8c322912`、任意距离挥空、攻击/命中/击退、动作/武器、移动/跳跃、画质、关节或正式资产，因此未新增浏览器/手机通过记录，也未重跑无关的黄金 Replay、输入 fuzz、专项生命周期或 soak。其余 Release producer/handoff、两个研究 Web App 与其余 G7 链仍待 strict 迁移；G7-G10 尚未完成，当前不可合并。

## G7.28 缺陷账本/报告/发布证据 strict 迁移证据

- Defect Ledger、派生 Report 与缺陷 Release producer 已迁入 strict `@number-strategy-jump/arena-release`，精确允许清单由 207 降至 205。缺陷 CLI、Stage 9 producer 与缺陷集成测试统一消费包公开 API，两份旧 JavaScript 真值已删除且没有兼容转发层。
- Ledger 继续绑定 40 位候选 commit、UTC 复核时刻、稳定匿名 reviewer/owner、完整性声明、缺陷与剩余风险。已解决缺陷必须同时具有解决摘要和验证引用；开放缺陷不得伪装已解决，且必须由只引用开放缺陷的具名剩余风险承接。Report 状态仍只由账本派生：未知问题不完整为 incomplete，开放 blocking/high 为 failed，其余为 ready，调用方不能手工写入状态。
- 原实现会在 10,000 项顶层集合上限检查前先深克隆，且单条 references/verificationReferences/defectIds 没有资源上限。本批改为先以数据描述符拒绝访问器、预检顶层集合，再把每组引用限制为 1,000 项后才深克隆；Release producer options 同样零 getter 读取并绑定 candidate commit。新增 strict 回归覆盖访问器零执行和超限引用失败关闭。
- 缺陷链定向 Node 4/4、完整 Node 698/698、strict package/治理测试 365/365（59 个测试文件）通过；ESLint、strict typecheck、JS 递减、产品依赖、Three 边界、本地生产依赖树、`git diff --check`、生产构建、Manifest、预算和生产产物边界均通过。本批没有新增或升级依赖；G7.25 已登记的 3 个 high 依赖风险继续作为 G8 阻断。
- 代码提交为 `778b7898fe2f9e069867f2ee312e0d28849b9df2`，clean build ID 为 `arena-778b7898fe2f-product`；Web/微信/抖音 delivery 为 `3807531 / 3835130 / 3835105 B`，JavaScript 为 `1463921 / 1507034 / 1507034 B`。三端 `sourceDirty=false`、默认入口均为 Product、Manifest 校验、预算和生产产物边界通过且 `freezeEligible=true`；交付字节与 G7.27 完全一致，缺陷治理链未进入默认 Product bundle。
- 本批没有修改已登记缺陷数据、状态阈值、研究数据、页面、游戏玩法、Bot、Replay/Profile schema 或权威运行结果，不改变 Gameplay V2 配置 hash `8c322912`、任意距离挥空、攻击/命中/击退、动作/武器、移动/跳跃、画质、关节或正式资产，因此未新增浏览器/手机通过记录，也未重跑无关的黄金 Replay、输入 fuzz、专项生命周期或 soak。其余 Release producer/handoff、两个研究 Web App 与其余 G7 链仍待 strict 迁移；G7-G10 尚未完成，当前不可合并。

## G7.29 构建/设备/真人公平性/Input Pilot 发布 producer strict 迁移证据

- Build Integrity/Budget、Stage 6/Stage 8/Performance Device、Human Fairness 与 Input Pilot 四类 Release producer 已迁入 strict `@number-strategy-jump/arena-release`，精确允许清单由 205 降至 201。Stage 9 与 Input Pilot 复验脚本、三组发布测试统一消费包公开 API，四份旧 JavaScript 真值已删除且没有兼容转发层。
- Build producer 继续要求 Web/微信/抖音三个唯一 Manifest 绑定同一 commit/build/sourceDirty 且默认入口为 Product，并以固定 Budget Policy 重算三端报告；输入数组现在通过属性描述符读取，既允许已经构造的 Manifest 实例，又不执行稀疏项/访问器。Device、Human Fairness 与 Input Pilot producer 的 options 全部先拒绝未知键和访问器，Input Pilot 的 Stage 6 依赖结果会再次无访问器克隆并核对同 commit/build、状态与 result hash。
- 架构门禁原先只扫描 `src/arena-release`，strict 迁移后会因旧目录文件数减少而产生假失败；现已同时扫描 `packages/arena-release/src`，继续禁止 Release 层依赖 Node、平台、Session、Renderer、Three、墙钟、随机或宿主全局。定向发布链 21/21、架构 36/36、完整 Node 698/698、strict package/治理测试 366/366（59 个测试文件）通过。
- ESLint、strict typecheck、JS 递减、产品依赖、Three 边界、本地生产依赖树、`git diff --check`、生产构建、Manifest、预算和生产产物边界均通过。包只增加仓内既有 device acceptance、performance evidence、stage9 evidence content、human study 与 input pilot 依赖，没有新增或升级外部依赖；G7.25 已登记的 3 个 high 依赖风险继续作为 G8 阻断。
- 代码提交为 `7c22527e9e5ebe9e6dd40522edbda0f548f6018c`，clean build ID 为 `arena-7c22527e9e5e-product`；Web/微信/抖音 delivery 为 `3807531 / 3835130 / 3835105 B`，JavaScript 为 `1463921 / 1507034 / 1507034 B`。三端 `sourceDirty=false`、默认入口均为 Product、Manifest 校验、预算和生产产物边界通过且 `freezeEligible=true`；交付字节与 G7.28 完全一致，离线发布 producer 未进入默认 Product bundle。
- 本批没有改变任一证据 Definition/Policy/阈值、研究数据、页面、游戏玩法、Bot、Replay/Profile schema 或权威运行结果，不改变 Gameplay V2 配置 hash `8c322912`、任意距离挥空、攻击/命中/击退、动作/武器、移动/跳跃、画质、关节或正式资产，因此未新增浏览器/手机通过记录，也未重跑无关的黄金 Replay、输入 fuzz、专项生命周期或 soak。仍依赖旧实验/回归模块的 Balance/Golden Replay/Regression producer、最终 handoff、两个研究 Web App 与其余 G7 链待迁移；G7-G10 尚未完成，当前不可合并。

## G6.49 研究 Web App strict 迁移与生命周期闭环证据

- Input Pilot 与 Human Match Study 两个研究 Web App 已迁为 strict TypeScript；G6.48 为临时迁移边界保留的两份 `.d.ts` 已随旧 JavaScript 真值一并删除，没有兼容转发层。精确 JavaScript 允许清单由 201 降至 199，两个 `.html` 仍只经已治理的薄启动入口装配对应 App，研究入口没有进入默认 Product 交付。
- 两个 App 对平台、Root、Definition、环境、构建身份、Controller、View、Runtime 和定时器所有权建立显式可空类型与释放检查；导出 revision、UTC 时间、匿名 ID、错误和宿主能力均在边界收窄。真人研究的并发启动现在只共享一个 Promise，构建身份加载期间销毁会拒绝迟到完成，不会在已销毁实例上继续写状态；Input Pilot 销毁失败会保留未释放的监听、Controller 或 View，下一次销毁只重试未完成所有权。
- 首轮真实页面验证发现 Pilot Workbench 的严格投影只接受缩减字段，错误拒绝 Controller 返回的合法完整 Workspace，页面因此进入启动失败。本批把 Workspace、Enrollment、Checkpoint 和 Assignment 的完整已知字段纳入投影白名单，同时仍拒绝未知字段和访问器；新增真实 `createInputPilotWorkspace()` 回归，避免只靠简化 fixture 再次漏检。
- 新增 3 项 Web App 生命周期回归，覆盖 Input Pilot 并发启动/启动中销毁、清理失败精确重试，以及 Human Match Study 并发启动/启动中销毁。完整 Node 为 701/701，strict package/治理为 366/366（59 个测试文件）；ESLint、strict typecheck、JS 递减、产品依赖、Three 边界、本地生产依赖树、`git diff --check`、生产构建、Manifest、预算和生产产物边界均通过。
