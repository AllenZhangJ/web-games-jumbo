...
- 状态：部分被 ADR-030 取代（第 4 节的灰盒生产回退构建）；其余已接受并实施（S8.5.5，G5.25 Surface 与 G5.30a ProductRenderer 完成 strict workspace 归真）
- 日期：2026-07-18
...
G5.30a 起，宿主无关 `ProductRenderer` 本体也位于 strict `@number-strategy-jump/arena-product-presentation`。它只持有经快照的 Gameplay Renderer/UI Surface 最小端口，不依赖 Three、DOM 或具体平台；具体 `ArenaGreyboxRenderer`、Web DOM Surface 和 Canvas/Three Surface 仍由 entry 组合注入。同步端口拒绝 thenable，构造/加载/销毁失败保留明确终态与可重试子资源所有权。
...
- ProductRenderer 对子端口方法建立不可变能力快照，并只向 Gameplay Renderer 暴露冻结的 UI overlay 包装；调用方后续篡改候选对象不能改写运行中能力。
