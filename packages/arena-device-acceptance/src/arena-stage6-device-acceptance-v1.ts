...
export function createArenaStage6DeviceAcceptanceV1Definition() {
  return createArenaDeviceAcceptanceDefinition({
    ...
    stage: 'stage6',
    checks: [
      { id: ARENA_STAGE6_DEVICE_CHECK_ID.CANCEL_CLEARS_INPUT, title: '触控取消后无残留 held/edge，必须重新触摸' },
      { id: ARENA_STAGE6_DEVICE_CHECK_ID.HIDE_RESUME_FRESH_TOUCH, title: '前后台不追帧，恢复后旧指针不复活' },
      { id: ARENA_STAGE6_DEVICE_CHECK_ID.LAUNCH_INTERACTIVE, title: '当前产物可启动并进入可操作的 1v1 对局' },
      { id: ARENA_STAGE6_DEVICE_CHECK_ID.MULTI_TOUCH_OWNERSHIP, title: '移动与动作的多指归属稳定，不串控件' },
      { id: ARENA_STAGE6_DEVICE_CHECK_ID.NO_HIDDEN_OPPONENT_LEAK, title: '界面、日志与结算不泄露 Bot 身份或难度' },
      { id: ARENA_STAGE6_DEVICE_CHECK_ID.RESOURCE_CLEANUP, title: '切局、返回和连续对局后无双 Session/双帧循环或阻断异常' },
      { id: ARENA_STAGE6_DEVICE_CHECK_ID.RESULT_REMATCH, title: '匹配、对局、结算与再来一局闭环可完成' },
      { id: ARENA_STAGE6_DEVICE_CHECK_ID.SAFE_AREA_PORTRAIT, title: '竖屏安全区、HUD 与底部控件无遮挡或不可触达区' },
      { id: ARENA_STAGE6_DEVICE_CHECK_ID.WEBGL_CONTEXT_RECOVERY, title: 'WebGL context loss 时暂停，恢复后不复活旧输入且不重复资源' },
    ],
    targets: [
      target('douyin-developer-tool', ARENA_DEVICE_ACCEPTANCE_PLATFORM.DOUYIN, ARENA_DEVICE_ACCEPTANCE_SURFACE.DEVELOPER_TOOL, COMMON_CHECKS),
      target('douyin-phone', ARENA_DEVICE_ACCEPTANCE_PLATFORM.DOUYIN, ARENA_DEVICE_ACCEPTANCE_SURFACE.PHYSICAL_DEVICE, PHYSICAL_CHECKS),
      target('web-phone', ARENA_DEVICE_ACCEPTANCE_PLATFORM.WEB, ARENA_DEVICE_ACCEPTANCE_SURFACE.MOBILE_BROWSER, PHYSICAL_CHECKS),
      target('wechat-developer-tool', ARENA_DEVICE_ACCEPTANCE_PLATFORM.WECHAT, ARENA_DEVICE_ACCEPTANCE_SURFACE.DEVELOPER_TOOL, COMMON_CHECKS),
      target('wechat-phone', ARENA_DEVICE_ACCEPTANCE_PLATFORM.WECHAT, ARENA_DEVICE_ACCEPTANCE_SURFACE.PHYSICAL_DEVICE, PHYSICAL_CHECKS),
    ],
  });
}
