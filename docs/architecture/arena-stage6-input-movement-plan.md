...
S6.1 的 Character 纯数据合同、比赛级内容快照、InputFrame V4、Replay V4 与严格旧版拒绝已经提交。S6.2 的 MovementRuntime/System、走跑投影、普通跳/蹲跳/二段跳/下砸、多通道 ActionResolver、多 lane ActionExecution、原子 Physics Port 和同源 ActionAffordance 已在当前候选工作区落地并通过本机 E1/E2 门禁。S6.3～S6.6 尚未实现；A/B Mapper 胜者仍未冻结。
...
- `BotObservation` 尚未投影 MovementSnapshot/ActionAffordance，Bot 也尚未主动生成 jump/slam；这是 S6.3 的唯一合法接入点。
- RawControlState、InputSampler、Gesture Recognizer 与 Mapper A/B 尚未实现。
- Arena 还没有触控适配器、灰盒 Renderer、HUD 或可视 Session 组合根。

因此 Stage 6 不能从触控手势或动画开始。先在无渲染权威层建立 Character、Movement 和语义输入合同，再接入 Bot，最后接触控与灰盒表现。
