...
- [x] 在 390×844 浏览器视口复核正式 Product 武器比较、六件研究武器卡和 5 道可读性任务；首次 4/5 的错误选择按同源击飞轴修正后达到 5/5，明确这只是脚本化任务闭环，不是真人样本；见[武器概览与可读性浏览器任务验证结果 V2](arena-v2-weapon-browser-task-validation-results-v2.md)。
...
- 生存最小循环已完成旧版无渲染原型：开局无武器、每 20 秒 3 把实体供给、10 轮临时等级成长、敌人压力增加、第一次复活/第二次结束；自动替换与未拾取 600 tick 回收仍待 Rule/Core 实现；
...
- [ ] 在真人测试中记录 3 分钟基础操作掌握、20 秒供给路线取舍、10 秒消失理解、第一次掉落后的再次开始和 30/60/120 分钟后的主动目标。
