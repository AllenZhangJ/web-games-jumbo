...
import { assertIntegerAtLeast, assertKnownKeys } from '../rules/definition-utils.js';
...
const COHORT_KEYS = new Set(['firstIndex', 'caseCount']);

export function createArenaStage9BotSeedCohort(value) {
  assertKnownKeys(value, COHORT_KEYS, 'Arena Stage 9 Bot seed cohort');
  const firstIndex = assertIntegerAtLeast(
    value.firstIndex,
    0,
    'Arena Stage 9 Bot seed cohort.firstIndex',
  );
  const caseCount = assertArenaExperimentCaseCount(
    value.caseCount,
    'Arena Stage 9 Bot seed cohort.caseCount',
  );
  ...
  return createSortedArenaExperimentSeeds(Array.from(
    { length: caseCount },
    (_, offset) => Number(
      (BigInt(firstIndex + offset) * step + base) & mask,
    ),
...
export function createArenaStage9BotSeeds(caseCountValue) {
  return createArenaStage9BotSeedCohort({
    firstIndex: 0,
    caseCount: assertArenaExperimentCaseCount(caseCountValue),
  });
}
