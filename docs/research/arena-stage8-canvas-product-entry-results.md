...
旧入口仍可执行：Web 提供 `/greybox.html`；小游戏同时产出 `game-product.js`、`game-greybox.js`，并支持 `npm run build:greybox` 选择灰盒 `game.js`。
...
- `npm run build:greybox` 后，两端 `game.js` 分别与本端 `game-greybox.js` 哈希一致；默认构建则与 `game-product.js` 一致。
