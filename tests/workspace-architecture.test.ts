...
test('workspace packages are private and follow the declared dependency direction', async () => {
  ...
  const persistence = JSON.parse(await readFile('packages/persistence/package.json', 'utf8'));
  const platform = JSON.parse(await readFile('packages/platform/package.json', 'utf8'));
  ...
  assert.equal(persistence.private, true);
  assert.equal(platform.private, true);
  ...
  assert.deepEqual(application.dependencies, {
    ...
    '@number-strategy/jump-engine': '0.1.0',
    '@number-strategy/persistence': '0.1.0',
  });
  ...
  assert.deepEqual(persistence.dependencies, {
    '@number-strategy/difficulty': '0.1.0',
    '@number-strategy/game-contracts': '0.1.0',
    '@number-strategy/gameplay': '0.1.0',
  });
  assert.deepEqual(platform.dependencies ?? {}, {});
