...
当前新增证据：规则层横向冲量已经接入轻量物理，在宽平台、KZ 灰盒窄路和边缘平台形成不同的“命中但安全 / 命中后出界”结果；固定侧移目标又验证了重锤挥空、锁链和冲锋盾命中的前摇差异。详见[武器地图边缘原型结果](arena-v2-weapon-map-prototype-results-v1.md)和[武器移动目标原型结果](arena-v2-weapon-moving-target-prototype-results-v1.md)。下一步加入攻击者失位、空中动作和多人拥挤，验证这些差异是否仍然可读且可反制。
