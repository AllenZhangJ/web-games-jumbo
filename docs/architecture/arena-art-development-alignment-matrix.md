...
- 硬门：总分≥90且每维度≥80%；当前两角色的角色/武器态/方向分别≥90%，任一角色×赤手或圆盾组合≥80%；历史候选记录的结果hash稳定。当前[候选基线](arena-art-silhouette-a0.3.md)为85/100、144/144在画面内，非真人代理整体及距离分层三项均≥90%；但真人0/10使该维度0%，且当前source identity尚待获准重生成/重绑定，因此`hardGatePassed=false`、A0.3仍`incomplete / pending-regeneration`。
...
- A0.3真人仍为`0/10`，A1.1必须按当前source重建；正式角色、武器、地图、VFX、HUD、音频、Blockout、截图、集成、设备、真人与Final全部关闭。本轮没有生成或修改任何资产，也未运行验证。
