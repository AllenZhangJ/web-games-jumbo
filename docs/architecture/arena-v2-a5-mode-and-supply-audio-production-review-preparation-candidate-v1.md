...
| 验证 | 直接规格已纳入SF-A3A6P.1固定runner；试听、波形、响度、拥塞、浏览器、设备、性能和生命周期仍顺延 |
...
源码构造时会拒绝13+4不闭合、Cue/Asset重复、path/bytes/SHA漂移、武器动作或战斗语法混入、供给优先级扩张、Queue/Voice预算漂移和伪造批准；直接规格已由固定runner执行。十项真实评审仍`not-run`，不能声明试听、因果辨识、混音、设备、静音、恢复或生命周期通过。
