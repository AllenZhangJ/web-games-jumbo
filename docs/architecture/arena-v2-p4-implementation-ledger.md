...
- 日期：2026-08-24。
- 当前状态：`P4.0-P4.3-static-candidates-landed / P4.4a-P4.4n-authority-feedback-hud-vfx-code-written-not-run / P4.4o-P4.4s-direction-contract-owner-and-resolution-code-written-not-run / P4.4t-atomic-feedback-direction-bundle-code-written-not-run / P4.4u-three-mode-runtime-session-checkpoint-direction-wired-not-run / P4.4v-validated-hud-direction-batch-wired-not-run / P4.4w-formal-three-authority-direction-vfx-wired-not-run / P4.4x-twenty-weapon-mount-and-feedback-anchor-wired-not-run / P4.4y-hud-primary-feedback-hierarchy-wired-not-run / P4.4z-authority-action-phase-weapon-readability-wired-not-run / P4.4aa-three-mode-first-screen-hud-language-wired-not-run / P4.4ab-twenty-weapon-audiovisual-production-manifest-code-written-not-run / P4.4ac-authority-weapon-phase-audio-identity-and-lifecycle-closure-code-written-not-run / P4.4ad-web-audio-sfx-master-limiter-bus-code-written-not-run / P4.4ae-single-weapon-production-readiness-matrix-code-written-not-run / P4.4af-single-weapon-evidence-score-and-audit-evaluator-code-written-not-run / P4.4ag-single-weapon-atomic-registration-plan-code-written-not-run / P4.4ah-single-weapon-immutable-registry-snapshot-builder-code-written-not-run / P4.4ai-single-weapon-cas-publication-and-exact-rollback-owner-code-written-not-run / P4.4aj-arena-v2-only-in-memory-registry-cas-port-code-written-not-run / P4.4ak-plain-data-persistence-envelope-code-written-not-run / P4.4al-dual-slot-persistent-registry-cas-port-code-written-not-run / P4.4am-persistent-registration-lifecycle-host-code-written-not-run / P4.4an-published-promotion-receipt-code-written-not-run / P4.4ao-atomic-registry-reference-code-written-not-run / P4.4ap-publish-swap-seal-coordinator-code-written-not-run / P4.4aq-durable-active-generation-marker-code-written-not-run / P4.4ar-activate-swap-seal-coordinator-code-written-not-run / P4.4as-orphan-pending-exact-recovery-code-written-not-run / P4.4at-active-bootstrap-code-written-not-run / P4.4au-registry-derived-survival-runtime-pool-code-written-not-run / P4.4av-registry-backed-three-mode-composition-code-written-not-run / P4.4aw-registry-backed-local-playable-owner-code-written-not-run / P4.4ax-active-reference-registration-assembly-code-written-not-run / P4.4ay-same-live-reference-promotion-owner-code-written-not-run / P4.4az-explicit-formal-web-registry-owner-and-promotion-surface-code-written-not-run / P4.4ba-partial-registry-information-weapon-availability-code-written-not-run / P4.4bb-active-registry-next-goal-eligibility-code-written-not-run / P4.4bc-empty-baseline-and-first-weapon-initialization-owner-code-written-not-run / P4.4bd-first-weapon-provisioning-and-runtime-owner-handoff-code-written-not-run / P4.4be-promotion-operation-projection-and-match-start-gate-code-written-not-run / P4.4bf-registry-availability-change-fact-code-written-not-run / P4.4bg-assessment-only-follow-up-and-reference-bypass-removal-code-written-not-run / P4.4bh-contiguous-registry-weapon-sequence-code-written-not-run / P4.4bi-bounded-coarse-promotion-orchestration-code-written-not-run / P4.4bj-long-restore-suffix-and-retained-resource-snapshot-code-written-not-run / P4.4bk-scheduled-long-suffix-and-cleanup-failure-injection-code-written-not-run / production-unreachable / hardGate=false`。
- 授权依据：[ADR-119](../decisions/119-arena-v2-continuous-development-with-deferred-gates.md)。开发连续推进；2026-08-24 当前 clean 基线`feature/arena-v2-design-docs@787ce27`已有`typecheck:app`与52包workspace build通过记录，P2候选测试记录为`351/421`且P3边界门通过。它们不构成P4定向测试、P4边界、性能、设备、真人或逐武器准入证据；这些门继续顺延并保持`not-run`。
...
- 结论边界：当前只证明版本化合同、20把静态Definition候选、20×10生存运行时变体，以及十四把扩展复用共享Rule/Physics和MatchCore Replay执行器的候选入口已落盘。P4.4c为十四把分别写入了防守方`direction/jump`真实反事实路径；P4.4d进一步为完整20把目录各写入高低差、多目标稳定排序和碰撞/边缘三类真实`Rule → Movement → Physics`探针，共60个具名probe；P4.4e把真实feedback owner/checkpoint接入Duel/Race/Survival的production-unreachable runtime和三模式QuickMatch composition能力身份；P4.4f让三个真实runtime都能从完整capability建立独立恢复分支，并用统一执行器比较同一下一帧的完整step、反馈事件、runtime/feedback checkpoint和state hash；P4.4g增加构造后、连续/恢复step、损坏checkpoint、未知事件和清理失败六个隔离注入点；P4.4h把Duel/Race精确一把选择与Survival完整20把武器池绑定到最终assignment、当前世界和完整runtime checkpoint；P4.4i把40个地面/空中动作、6类基础战斗语法、5类反馈和3种模式正规化为483个可解析读取组合，其中480个为武器动作反馈、3个为全局移动失足反馈；P4.4bj再把恢复比较扩为2–240个连续非终局tick，逐tick核对完整step、反馈、runtime/content/checkpoint、state hash与真实retained resource snapshot，并要求恢复分支销毁后资源与提交记录均为零；P4.4bk把六类既有故障和两类资源读取故障统一为最多8项的调用序号计划，可精确落在长后缀中间，并可让destroy与销毁后资源读取同时失败以保留聚合清理证据。上述代码均未运行，尚不知哪些武器在当前场景时序中形成可区分观察结果，也没有三模式恢复/失败关闭/选择/表现读取矩阵的动态通过证据。所有运行结果均为`not-run`，不得宣称P4完成、20把均已深度验证、正式VFX/音频已制作或武器已经平衡。

- P4.4bm边界：P4.4bk的计划已固定为三模式共享的33场景重放清单和无堆栈、墙钟、设备数据的确定性统一报告；现已接真实三模式runtime与固定中性InputFrame工厂，但清单仍未运行。
...
| P4.2 无渲染规则后果 | 静态候选已落盘，未执行 | 六把首发24场景与十四把扩展56场景，共80个地面/空中×边缘/窄路共享Rule/Physics场景；20把均有MatchCore/Replay/checkpoint候选，六把首发另有专属语义探针 | 首轮执行、互撞、多seed、长局、失败注入及正式Mode lifecycle |
| P4.3 Bot与生存等级 | P4.3a/P4.3b/P4.3c/P4.3d/P4.3e静态候选已落盘，未执行 | 10级×20把运行时Definition；20秒首刷/间隔、每波仍只生成3把、10波轮转覆盖20把、10秒消失；真实拾取/替换/过期；基线三把1级/10级真实Rule/Physics；Bot只读取射程、就绪、最小蓄势tick与当前权威蓄势进度并输出普通InputFrame；tier policy解析V3收藏/运行时/等级身份并升级MatchReadFrame；10阶段敌人数`1→16`；P3.4b隔离shared-world候选已在同一tick中消费供给、V2 Bot输入、Rule/Physics和Survival Mode命令 | 扩展武器Bot/Rule矩阵、默认生产Mode组合、集中验证、失败注入和数值验收 |
| P4.4 Presentation与正式资产 | P4.4a–P4.4ae代码已落盘，未执行 | 五类因果语义、V6事件、双checkpoint原子owner、Duel/Race/Survival同批冲量方向、Session/Product/HUD只读方向事实、20武器Cue读取、20把统一手持/地面变换、权威动作阶段材质/局部变换、身体/武器VFX锚点、主次命中反馈卡、三模式首屏目标语言、正式Three方向投影与专用Cue→5张候选纹理映射；20把附件/动作/挂点/480反馈配方与60份前摇/释放/恢复音频候选已进入清单和Stage owner；阶段音频现严格闭合动作事件、装备实例、收藏/运行时武器、生存等级与事件水位；逐把生产准入矩阵分别绑定Definition/反制/动作表现/附件/命中与阶段音频/VFX、证据缺口和独立回滚单元；未批准新增正式资产 | 定向执行证据、失败清理矩阵、20把选择/方向/表现矩阵、60份阶段音频批准、VFX批准、低动效、三端和真人理解 |
| P4.4c 防守方反事实执行 | 代码已写，未运行验证 | control/counterfactual只改变defender movement输入；Grammar限制`direction/jump`；真实Movement command、物理位移、支撑/掉落、命中与首次命中tick证据绑定14把逐把probes | 执行定向测试，列出仍无可区分结果的具名武器，再进入数值/真人验收 |
| P4.4d 20把空间与多目标反制 | 代码已写，未运行验证 | 完整收藏目录×高低差/多目标/碰撞边缘共60个probe；真实装备拾取、Action选择、Movement、Physics、冲量、支撑/掉落；只置换候选顺序或defender方向/跳跃输入 | 集中验证后列出observed/indistinguishable/deferred矩阵，再调整场景和数值 |
| P4.4f/g/h/bj/bk 恢复后缀、失败关闭与选择闭包 | 代码已写，未运行验证 | 三模式真实runtime可从完整capability分叉；V1比较同一下一帧，V2比较2–240个连续非终局tick的完整step/反馈/checkpoint/content/state hash与真实retained resource snapshot；活动分支固定持有authority与mode driver，恢复分支销毁后要求资源和提交记录归零；六类既有故障和连续/恢复资源读取故障可按调用序号组成最多8项计划，支持destroy与销毁后读取双失败聚合；端口只接受数据方法；未知V6事件直接失败；Duel/Race精确一把与Survival完整20把选择绑定最终assignment及runtime checkpoint | 集中执行八点×三模式×2/60/240 tick矩阵、20把×Duel/Race选择矩阵、生存目录闭包及资源清理反证 |
| P4.4i/j/k 二十武器三模式反馈读取与HUD宿主 | 代码已写，未运行验证 | 20个独立反馈签名、40个动作绑定、6个核心动词表现族、5类语义、3个模式焦点；解析容量480个武器动作反馈组合加3个全局移动失足组合；显式适配器和候选epoch宿主把武器专属Cue送入既有HUD反馈队列和效果consumer，保持原有epoch、去重、预算与清理owner，并把读取计划缓存限制在当前epoch/64项seen窗口 | 集中执行483组合、三模式epoch恢复与失败矩阵、逐把Concept/Blockout/Integration/Final、音画资产与真人归因 |
| P4.4bl 三模式可重放故障报告 | 代码已写，未运行验证 | 固定120 tick、每模式11场景，覆盖fork、连续/恢复步进、capability损坏、未知事件、两侧资源读取、destroy、destroy后读取和双清理失败，共33项；统一报告只保存错误名/消息/有界Aggregate子项、触发证据、清理结果和确定性hash，逐场runtime由装配器独占销毁 | 接入真实三模式runtime/InputFrame工厂并执行清单；默认Registry、Composition和入口继续断开 |
| P4.4bm 真实三模式故障清单装配 | 代码与未运行测试源码已写 | 复用正式QuickMatch roster/content/final assignment规则，按固定seed建立并启动真实Duel 1+1、Race 1+3、Survival 1+16 runtime；每个场景生成120个连续本地中性InputFrame并交给P4.4bl，调用方独占销毁，导入无副作用 | 首次执行33场景；确认120 tick内三模式均保持可比较、全部计划精确触发、每例资源归零及报告hash稳定；默认Registry/Composition/entry继续断开 |
...
- P4.4bm已把P4.4bl的33场景清单接到真实三模式工厂和固定输入；当前仍未执行，不能宣称故障计划触发、资源归零或报告hash通过。
...
- 阶段判断：`remain`；原因是当前只有静态开发候选，没有任何可用于评分或晋级的运行证据。
