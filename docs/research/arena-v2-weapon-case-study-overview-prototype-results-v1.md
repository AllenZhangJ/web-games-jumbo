...
- 状态：研究工具链原型通过；六件逐动作深研案例均已接入独立研究 Definition、数值投影、主战斗语言和三步学习路径，六件均已有专属地面/空中 Replay，延迟/预警/障碍等复杂行为仍保留独立研究边界
- 日期：2026-07-28
...
- 地图关系信号和动作级反制集合；
- 一个主战斗语言（封路、换位、直线压制、绕后、读招反制或延迟重击），以及该语言要求的关键数值轴、地图空间和反制方式；
...
主战斗语言负责横向比较，学习路径负责纵向学习。六件案例各绑定一个不同的主语言；组合阶段会检查该语言的全部关键轴是否存在于案例审计，缺失即失败，不允许只靠文字标签掩盖数值研究缺口。白金双枪因此补充了“横向击飞”轴，继续保留远程覆盖与近身恢复的差异。
...
主战斗语言绑定顺序固定为：魔血镰刃—封路、真·哈迪斯钩镰—换位、白金双枪—直线压制、血影钩刃—绕后、幻虎巨拳—读招反制、猛犸石斧—延迟重击。该绑定来自研究推导，不代表原作官方分类，也不进入生产装备身份。

魔血镰刃的宽覆盖、active 前离开攻击线和平台边缘击落，猛犸石斧的长前摇预判失败、命中和平台边缘击落，已经由真实 MatchCore/MatchReplay 验证；两者延迟危险区仍保持独立研究假设。血影钩刃的方向分支已经由 `rear-cone` 与目标主动转身 Replay 验证；障碍阻挡仍由独立二维探针验证，不会被伪装成现有 9 个主比较轴。真·哈迪斯钩镰已经由真实 MatchCore/MatchReplay 验证提前释放、到期取消、提交后命中和平台边缘击落，但这只证明“承诺与支撑面后果”可以被权威链路读取，不等同于原作的完整反弹、多阶段追击或正式平衡。障碍依赖仍需接入真实地图表面、空中高度和复活重新进入路线后，才有资格讨论是否新增玩家可读字段。
...
- `arena-v2-weapon-case-study-language.ts`：六件案例的主战斗语言绑定、关键数值轴和地图/反制读出；
...
- 断言还覆盖六件案例一对一主战斗语言、语言关键轴必须存在于案例审计、白金双枪横向击飞轴和研究页的语言/关键数值读出；
