...
| `movement.semantic-actions` | 地面跳、二段跳、蹲跳、空中攻击/落地 | 1834352640 | 836 | `b68c763e` | `ee341734` |

Manifest hash 为 `0dace228`。完整 JSON 有意保留输入、checkpoint、事件与结果的人工可审查性；只有仓库体积出现可测量问题时才评估压缩归档格式。
...
- 全量测试：696/696 通过。
- 黄金语料：4/4 严格重放与再生成，Manifest hash `0dace228`。
- input fuzz：三套 Mapper 各 40 局，120/120 唯一最终 hash，6/6 抽样严格回放。
- 生命周期回归：91/91 通过。
- 旧表现 Session soak：100/100，最终 frame/listener/input 所有权均为 0/未绑定，GC 后堆增长 2,457,760 B，低于 8 MiB 预算。
- Product Presentation Session soak：100/100、100 个唯一 Authority hash，最终资源所有权清零，GC 后堆增长 5,959,360 B，低于 8 MiB 预算。
